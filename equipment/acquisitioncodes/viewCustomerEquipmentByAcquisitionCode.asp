...
<h1 class="page-header">View Customer Equipment Acquisition Code Details For <strong><%= GetAcquisitionCodeByIntRecID(InternalRecordIdentifier) %> - <%= GetAcquisitionCodeDescByIntRecID(InternalRecordIdentifier) %></strong></h1>

<div class="row">
 	<div class="col-lg-12">
	 	<p><a href="<%= BaseURL %>equipment/acquisitioncodes/main.asp"><button type="button" class="btn btn-success">Return To Main Acquisition Codes Screen</button></a></p>
 	</div>
</div>
	...
	<div class="table-responsive">
            <table class="table table-striped table-condensed table-hover table-bordered sortable">
              ...
              <tbody class='searchable'>
				...
				SQLCustomerEquipment = SQLCustomerEquipment & " WHERE EQ_Equipment.CurrentConditionIntRecID = " & InternalRecordIdentifier & " ORDER BY EQ_Equipment.ModelIntRecID "
				...
				If NOT rsCustomerEquipment.EOF Then
					...
					Do While Not rsCustomerEquipment.EOF
						...
						acquisitionCodeIntRecID = rsCustomerEquipment("AcquisitionCodeIntRecID")
						
						If acquisitionCodeIntRecID <> "" Then
						
							SQLEquipAcquisitionCode = "SELECT * FROM EQ_AcquisitionCodes WHERE InternalRecordIdentifier = " & acquisitionCodeIntRecID
								
							Set cnnEquipAcquisitionCode = Server.CreateObject("ADODB.Connection")
							cnnEquipAcquisitionCode.open (Session("ClientCnnString"))
							Set rsEquipAcquisitionCode = cnnEquipAcquisitionCode.Execute(SQLEquipAcquisitionCode)
							
							If NOT rsEquipAcquisitionCode.EOF Then
								acquisitionCode = rsEquipAcquisitionCode("acquisitionCode")
								acquisitionCodeDesc = rsEquipAcquisitionCode("acquisitionDesc")
							Else
								acquisitionCode = ""
								acquisitionCodeDesc = ""
							End If
			        ...
			        %>
						...
						<tr>
							...
							<td><%= acquisitionCode %> - <%= acquisitionCodeDesc %></td>
