...
If newViewName <> "" AND originalViewName <> "" Then
	...
	dummy = MUV_WRITE("CRMVIEWSTATERECPOOL",newViewName)
	...
	SQLReportSettings = "UPDATE Settings_Reports SET UserReportName = '" & newViewNameSaveSQL & "' WHERE ReportNumber = 1400 AND UserNo = " & Session("userNo") & " AND PoolForProspecting = 'Dead' AND UserReportName = '" & originalViewNameSaveSQL & "'"	
	...
	Description = GetUserDisplayNameByUserNo(Session("Userno")) & " renamed the " & GetTerm("Prospecting") & "  " & GetTerm("Recycle Pool") & " filter view named, " & originalViewNameSaveSQL & ", to " & newViewNameSaveSQL & "."
