...
If Request.Form("updateAction")="save" Then
	...
	If not rs.EOF Then
		...
		Orig_Note = rs("Note")
		Orig_NoteType = GetNoteTypeByNum(rs("NoteTypeNumber"))
	...
	NoteType 	= GetNoteTypeByNum(Request.Form("LogNoteTypeNumber"))
	...
	If Orig_NoteType <> NoteType Then
		...
		Description =  "Prospect note type changed from " & Orig_NoteType & " to " & NoteType & " for the prospect " & GetProspectNameByNumber(ProspectIntRecID)
		CreateAuditLogEntry GetTerm("Prospecting") & " note type change ",GetTerm("Prospecting"),"Minor",0,Description
		
		Description =  "Note type changed from <em><strong>" & Orig_NoteType & "</em></strong> to <em><strong>" & NoteType & "</em></strong>"
		Record_PR_Activity ProspectIntRecID, Description, Session("UserNo")

	End If

	Query = "UPDATE PR_ProspectNotes SET NoteTypeNumber='"&Request.Form("LogNoteTypeNumber")&"', Note='"&EscapeSingleQuotes(Request.Form("LogNote"))&"', "
...
If Request.Form("updateAction")="insert" Then
	...
	Query = Query & "VALUES (" & ProspectIntRecID & ", getdate(), " &Session("Userno")& ", '"&Request.Form("LogNoteTypeNumber")&"', "
...
If Request.Form("updateAction")="delete" Then
	...
	If not rs.EOF Then
		...
		Note = rs("Note")
		NoteType = GetNoteTypeByNum(rs("NoteTypeNumber"))
	...
	Description = "The note " & Note  & " of type " & NoteType & ", originally entered by " & UserName & " on " & FormatDateTime(DateAndTime,2) & " for prospect " & GetProspectNameByNumber(ProspectIntRecID) & " was deleted."
	CreateAuditLogEntry GetTerm("Prospecting") & " prospect note deleted ",GetTerm("Prospecting"),"Minor",0,Description
	
	Description = "The note <em><strong> " & Note  & "</em></strong> of type <em><strong>" & NoteType & "</em></strong>, originally entered by " & UserName & " on " & FormatDateTime(DateAndTime,2) & " for this prospect was deleted."
	Record_PR_Activity ProspectIntRecID, Description, Session("UserNo")
...
Query = "SELECT n.*, n.InternalRecordIdentifier as id, nt.NoteType, u.userDisplayName FROM PR_ProspectNotes n LEFT OUTER JOIN "
Query = Query & " PR_NoteTypes nt ON n.NoteTypeNumber = nt.InternalRecordIdentifier LEFT OUTER JOIN tblUsers u ON n.EnteredByUserNo = u.UserNo "
Query = Query & " WHERE ProspectIntRecID = " & ProspectIntRecID & " ORDER BY Sticky Desc, DateAndTime Desc"		


'Query = "SELECT DetailType, id, DateAndTime, NoteTypeNumber, EnteredByUserNo, Note, Sticky, StageNumber, ActivityStatus, ActivityNumber FROM "
'Query = Query & "(SELECT 'Note' AS DetailType, InternalRecordIdentifier as id, DateAndTime, NoteTypeNumber, EnteredByUserNo, Note, Sticky, 0 as StageNumber, '' AS ActivityStatus, 0 as ActivityNumber "
'Query = Query & "FROM PR_ProspectNotes WHERE ProspectIntRecID = " & ProspectIntRecID & " "
'Query = Query & "UNION "
'Query = Query & "SELECT 'Activity' AS DetailType, InternalRecordIdentifier as id, RecordCreationDateTime, 0 AS Expr1, StatusChangedByUserNo, Notes AS Expr2, 0 AS Expr3, 0 as StageNumber, Status, ActivityRecID "
'Query = Query & "FROM PR_ProspectActivities WHERE ProspectRecID = " & ProspectIntRecID & " "
'Query = Query & "UNION "
'Query = Query & "SELECT 'Stage Change' AS DetailType,InternalRecordIdentifier as id, RecordCreationDateTime, 0 AS Expr1, StageChangedByUserNo As UserNo, Notes AS Expr2, 0 AS Expr3, StageRecID AS StageNumber, '' AS ActivityStatus, 0 as ActivityNumber   "
'Query = Query & "FROM PR_ProspectStages AS PR_ProspectStages_1 WHERE ProspectRecID = " & ProspectIntRecID & ") AS t1 "
...
Query = Query & "UNION "
Query = Query & "SELECT 'Email' AS DetailType,InternalRecordIdentifier as id, RecordCreationDateTime, 0 AS Expr1, 0 As UserNo, '' AS Expr2, Sticky, 0 AS StageNumber, '' AS ActivityStatus, 0 as ActivityNumber   "
...
If not rs.EOF Then
	...
	Do While Not rs.EOF
			...
			Response.Write(",""LogNoteTypeNumber"":""" & EscapeQuotes(rs("NoteTypeNumber")) & """")
