...
If reportNameToSave <> "" Then
	...
	dummy = MUV_WRITE("CRMVIEWSTATERECPOOL",reportNameToSave)
	...
	SQLDeleteFilter = "DELETE FROM Settings_Reports WHERE ReportNumber = 1400 AND UserNo = " & Session("userNo") & " AND PoolForProspecting = 'Dead' AND UserReportName = '" & reportNameToSaveSQL & "'"
	Set rsReportSettings = cnnReportSettings.Execute(SQLDeleteFilter)
	
	SQLReportSettings = "UPDATE Settings_Reports SET UserReportName = '" & reportNameToSaveSQL & "' WHERE ReportNumber = 1400 AND UserNo = " & Session("userNo") & " AND PoolForProspecting = 'Dead' AND UserReportName='Current'"
	...
	Description = GetUserDisplayNameByUserNo(Session("Userno")) & " created the " & GetTerm("Prospecting") & "  " & GetTerm("Recycle Pool") & "filter view named, " & reportNameToSave
