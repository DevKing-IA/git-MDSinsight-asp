...
'******************************************************************************************************************
'This will give us an array of the internal record identifiers of the selected LOST stage checkboxes
'******************************************************************************************************************
selectedLostStagesToFilter = Request.Form("chkStageLost")

'******************************************************************************************************************
'We need to see if the user selected any LOST stages
'******************************************************************************************************************
selectedLostStagesToFilterArray = Split(selectedLostStagesToFilter,",")
upperBoundLostStages = ubound(selectedLostStagesToFilterArray)
stagesToFilterLost = "" 

For i=0 to upperBoundLostStages
   If selectedLostStagesToFilterArray(i) <> "" Then
   		stagesToFilterLost = stagesToFilterLost & selectedLostStagesToFilterArray(i) & ","
   End If
Next

If stagesToFilterLost <> "" Then
	stagesToFilterLost = Left(stagesToFilterLost, Len(stagesToFilterLost)-1)
End If
...
If stagesToFilterLost <> "" Then
	ReportSpecificData23 = stagesToFilterLost
Else
	ReportSpecificData23 = ""
End If

'*****************************************************
'Did the user select the filter, "Where prospect was LOST in a DATE range"'
'*****************************************************

optStageLostDateRange = Request.Form("optStageLostDateRange")

If optStageLostDateRange = "WasLostInDateRange" Then

	selLostStageDateRangeCustom = Request.Form("selLostStageDateRangeCustom")
	txtStageLostDateRangeStartDate = Request.Form("txtStageLostDateRangeStartDate")
	txtStageLostDateRangeEndDate = Request.Form("txtStageLostDateRangeEndDate")
	
	If selLostStageDateRangeCustom <> "" Then
		ReportSpecificData23a = selLostStageDateRangeCustom
		ReportSpecificData24 = ""
		ReportSpecificData25 = ""
	Else
		ReportSpecificData23a = ""
		ReportSpecificData24 = txtStageLostDateRangeStartDate
		ReportSpecificData25 = txtStageLostDateRangeEndDate	
	End If
Else
	ReportSpecificData23a = ""
	ReportSpecificData24 = ""
...
'******************************************************************************************************************
'This will give us an array of the internal record identifiers of the selected Unqualified stage checkboxes
'******************************************************************************************************************
selectedUnqualifiedStagesToFilter = Request.Form("chkStageUnqualified")

'******************************************************************************************************************
'We need to see if the user selected any Unqualified stages
'******************************************************************************************************************
selectedUnqualifiedStagesToFilterArray = Split(selectedUnqualifiedStagesToFilter,",")
upperBoundUnqualifiedStages = ubound(selectedUnqualifiedStagesToFilterArray)
stagesToFilterUnqualified = "" 

For i=0 to upperBoundUnqualifiedStages
   If selectedUnqualifiedStagesToFilterArray(i) <> "" Then
   		stagesToFilterUnqualified = stagesToFilterUnqualified & selectedUnqualifiedStagesToFilterArray(i) & ","
   End If
Next

If stagesToFilterUnqualified <> "" Then
	stagesToFilterUnqualified = Left(stagesToFilterUnqualified, Len(stagesToFilterUnqualified)-1)
End If
...
If stagesToFilterUnqualified <> "" Then
	ReportSpecificData26 = stagesToFilterUnqualified 
Else
	ReportSpecificData26 = ""
End If

'*****************************************************
'Did the user select the filter, "Where prospect was Unqualified in a DATE range"'
'*****************************************************

optStageUnqualifiedDateRange = Request.Form("optStageUnqualifiedDateRange")

If optStageUnqualifiedDateRange = "WasUnqualifiedInDateRange" Then

	selUnqualifiedStageDateRangeCustom = Request.Form("selUnqualifiedStageDateRangeCustom")
	txtStageUnqualifiedRangeStartDate = Request.Form("txtStageUnqualifiedRangeStartDate")
	txtStageUnqualifiedRangeEndDate = Request.Form("txtStageUnqualifiedRangeEndDate")
	
	If selUnqualifiedStageDateRangeCustom <> "" Then
		ReportSpecificData26a = selUnqualifiedStageDateRangeCustom
		ReportSpecificData27 = ""
		ReportSpecificData28 = ""
	Else
		ReportSpecificData26a = ""
		ReportSpecificData27 = txtStageUnqualifiedRangeStartDate
		ReportSpecificData28 = txtStageUnqualifiedRangeEndDate
	End If
Else
	ReportSpecificData26a = ""
	ReportSpecificData27 = ""
...
If rs.EOF Then
	SQL = "Insert into Settings_Reports (ReportNumber, UserNo, PoolForProspecting) Values (1400, " & Session("userNo") & ", 'Dead')"
	rs.Close
	Set rs= cnn8.Execute(SQL)
End If
...
If ReportSpecificData2 = "" AND ReportSpecificData3 = "" AND ReportSpecificData4 = "" AND ReportSpecificData5 = "" AND _
	...
	%>
	<form id="frmClearFilterView" name="frmClearFilterView" method="POST" action="mainRecyclePool.asp">
...
Else
	...
	SQL = "SELECT * FROM Settings_Reports WHERE ReportNumber = 1400 AND UserNo = " & Session("userNo") & " AND PoolForProspecting = 'Dead' AND UserReportName = 'Current'"
	...
	'Rec does not exist yet, make it quick but empty, update it later
	If rs.EOF Then
		SQL = "INSERT INTO Settings_Reports (ReportNumber,UserNo,UserReportName,PoolForProspecting) Values (1400," & Session("userNo") & ",'Current','Dead')"
		rs.Close
		Set rs= cnn8.Execute(SQL)
	End If
	...
	SQL = SQL & " WHERE ReportNumber = 1400 AND UserNo = " & Session("userNo") & " AND PoolForProspecting = 'Dead' AND UserReportName = 'Current'"
	Set rs= cnn8.Execute(SQL)
	...
	Set rs = Nothing
	Set cnn8 = Nothing
	dummy = MUV_WRITE("CRMVIEWSTATERECPOOL","Current")
	%>
	
	<form id="frmClearFilterView2" name="frmClearFilterView2" method="POST" action="mainRecyclePool.asp">
