...
<h1 class="page-header">Add / Edit Note Types</h1>

<div class="row">
 	<div class="col-lg-12">
	 	<p>
 <a href="addNoteType.asp">
    	<button type="button" class="btn btn-success">Add New Note Type</button>
	</a>
	 	</p>
	...
	<div class="table-responsive">
            <table    class="table table-striped table-condensed table-hover table-bordered sortable">
              ...
              <tbody class='searchable'>
				...
				SQL = "SELECT * FROM PR_NoteTypes order by NoteType"
				...
				If not rs.EOF Then
			        ...
			        %>
						<!-- table line !-->
						<tr>
							<!-- <td><a href='editNoteType.asp?i=<%= rs.Fields("InternalRecordIdentifier")%>'><%= rs.Fields("NoteType")%></a></td> !-->
							
							<td>
								<!-- link for modal !-->
									<a  data-toggle="modal" data-target="#myModal" class="modal-link"><%= rs.Fields("NoteType")%></a>
								<!-- eof link for modal !-->
 							</td>
							
							<td><a href='deleteNoteTypeQues.asp?i=<%=rs.Fields("InternalRecordIdentifier")%>'><i class="fas fa-trash-alt"></i></a></td>
					   	</tr>
					<%
						rs.movenext
					loop
