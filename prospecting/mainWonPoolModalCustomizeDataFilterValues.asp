...
<!--
	...
	function isOneStageWonChecked() {
	    return ($('[name="chkStageWon"]:checked').length > 0);
	}
	...
	function validateCustomizeForm()	{
		...
		if ($('#optStageWonDateRange').is(':checked'))
		{
			if ($("#selWonStageDateRangeCustom").find(":selected").val() == "") 
			{
				//Date Range Checking Here
				if (document.frmProspectingCustomizeDataFilters.txtStageWonDateRangeStartDate.value == "" || 
					document.frmProspectingCustomizeDataFilters.txtStageWonDateRangeEndDate.value == "") 
					{		
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
		...
		$("#btnClearStagesWon").click(function() {
			$('input[name="optStageWonDateRange"]').prop('checked', false);
			$("#txtStageWonDateRangeStartDate").val('');
			$("#txtStageWonDateRangeEndDate").val('');
			$('#selWonStageDateRangeCustom').children().removeProp('selected');
			$("#stageRangeWon").val('');
		});	
		...
		$("#selWonStageDateRangeCustom").change(function() {
		    ...
		    if (selectedValue != "") 
		    {
		    	...
		    	$("#txtStageWonDateRangeStartDate").val('');
		    	$("#txtStageWonDateRangeEndDate").val('');
		    	$("#optStageWonDateRange").prop("checked","checked");
		    }
		    else
		    {
		    	$("#optStageWonDateRange").prop("checked","");
		    }
		});
...
<!-- modal box !-->
<div class="modal fade bs-modal-filter-prospecting-data-won-pool" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg modal-height">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title" id="myModalLabel" align="center">Customize and Filter <%= GetTerm("New Customer Pool") %> Screen</h4>
			</div>
			<%
			'************************
			...
			SQL = "SELECT * FROM Settings_Reports WHERE ReportNumber = 1400 AND UserNo = " & Session("userNo") & " AND PoolForProspecting = 'Won' AND UserReportName = '" & SQLReportName & "'"
	...
	<form method="post" action="mainWonPoolCustomizeSaveDataFilterValues.asp" id="frmProspectingCustomizeDataFilters" name="frmProspectingCustomizeDataFilters" onsubmit="return validateCustomizeForm();">
  	...
  	<div class="container-fluid container-modal">
      	<div class="row">
	      	...
	      	<div class="col-lg-2 col-md-3 col-sm-12 col-xs-12 left-column <% If ReportSpecificData23 <> "" OR ReportSpecificData24 <> "" Then Response.Write("activefilter") %>">
 		      	<h4><br>Stage - Won<br><br><button type="button" class="btn btn-info btn-xs btn-block" id="btnClearStagesLost">clear selections</button></h4>
	      	</div>
	      	<!-- eof left column !-->
		      	...
		      	<div class="row" style="margin-top:35px;">
				    ...
				    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				    	...
				    	<div class="col-lg-12" style="margin-left:-15px;">
					    	<input type="radio" id="optStageWonDateRange" name="optStageWonDateRange" value="WaWonInDateRange" <% If ReportSpecificData24 <> "" AND ReportSpecificData25 <> "" Then Response.write("checked") %>>
							<%
							If ReportSpecificData24 <> "" AND ReportSpecificData25 <> "" Then
								startDateStageWonRange = dateCustomFormat(ReportSpecificData24)
								endDateStageWonRange = dateCustomFormat(ReportSpecificData25) 
							Else
								thirtyDaysAgo = DateAdd("d",-30, DateSerial(Year(Now()), Month(Now()), Day(Now())))
								startDateStageWonRange = dateCustomFormat(thirtyDaysAgo)
								endDateStageWonRange = dateCustomFormat(Now())
							End If
	
							%>
							Where prospect was "won" in the date range:
						</div> 	
				    </div>	
				 </div>
		      	...
		      	<div class="row" style="margin-top:15px;">
				    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-left:15px;">
				    	...
				    	<div class="col-lg-5" style="margin-left:-15px;">
							...
							<select class="form-control" name="selWonStageDateRangeCustom" id="selWonStageDateRangeCustom">
						...
						<div class="col-lg-6">
                            ...
                            <input type="text" id="stageRangeWon" class="form-control">
                            <i class="glyphicon glyphicon-calendar fa fa-calendar stagerangedatepicker"></i>	
                        </div>	
						<input type="hidden" id="txtStageWonDateRangeStartDate" name="txtStageWonDateRangeStartDate" value="<%= startDateStageWonRange %>">
						<input type="hidden" id="txtStageWonDateRangeEndDate" name="txtStageWonDateRangeEndDate" value="<%= endDateStageWonRange %>">
				    </div>	
				 </div>
		      	...
		      	<select class="form-control" name="selProspectOwnerUserNo" id="selProspectOwnerUserNo" multiple="multiple">
	  	  			<%
		     			...
		     			End If
			      	  	...
			      	  	SQLOwner = SQLOwner & "WHERE UserNo IN (SELECT DISTINCT OwnerUserNo FROM PR_Prospects WHERE Pool='Won') "
		      	...
		      	<select class="form-control" name="selProspectCreatedByUserNo" id="selProspectCreatedByUserNo" multiple="multiple">
	  	  			<%
		     			...
		     			End If
			      	  	...
			      	  	SQLCreatedBy = "SELECT DISTINCT(CreatedByUserNo) FROM PR_Prospects WHERE Pool='Won' ORDER BY CreatedByUserNo DESC"
...
<script type="text/javascript">
		...
		startDateStageWonRange = moment(Date.parse($('#txtStageWonDateRangeStartDate').val()));
		endDateStageWonRange = moment(Date.parse($('#txtStageWonDateRangeEndDate').val()));
		...
		$('#stageRangeWon').daterangepicker({
            ...
            function (start, end) {
                $('#txtStageWonDateRangeStartDate').val(start.format('MM/DD/YYYY'));
                $('#txtStageWonDateRangeEndDate').val(end.format('MM/DD/YYYY'));
				$("#optStageWonDateRange").prop("checked","checked");
				$("#selWonStageDateRangeCustom").val('');
            }
        );
		...
		$('#stageRangeUnqualified').daterangepicker({
                opens: 'right',
                startDate: startDateStageWonRange,
                endDate: endDateStageWonRange,
                alwaysShowCalendars: true,
                timePicker: false,
                linkedCalendars: false,
                autoUpdateInput:true,
		...
		$('#txtStageWonDateRangeStartDate').val(startDateStageWonRange.format('MM/DD/YYYY'));
		$('#txtStageWonDateRangeEndDate').val(endDateStageWonRange.format('MM/DD/YYYY'));
