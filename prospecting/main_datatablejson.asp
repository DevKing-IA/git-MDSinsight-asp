...
If owner="" Or Not IsNumeric(owner) Then
	owner = 0
End if
owner = CLng(owner)
...
%>	
	...
	SQL8 = SQL8 & "PR_Prospects.LeadSourceNumber,PR_Prospects.IndustryNumber,PR_Prospects.EmployeeRangeNumber,PR_Prospects.OwnerUserNo, "
	SQL8 = SQL8 & "PR_Prospects.CreatedDate,PR_Prospects.CreatedByUserNo,PR_Prospects.TelemarketerUserNo,PR_Prospects.NumberOfPantries,"
	SQL8 = SQL8 & "PR_LeadSources.LeadSource,PR_Stages.Stage,PR_ProspectStages.RecordCreationDateTime,PR_Industries.Industry, "
	SQL8 = SQL8 & "PR_Activities.Activity, PR_ProspectActivities.ActivityDueDate "
...
SQL8 = SQL8 & " INNER JOIN PR_ProspectActivities ON PR_Prospects.InternalRecordIdentifier=PR_ProspectActivities.ProspectRecID"
SQL8 = SQL8 & " INNER JOIN PR_Activities ON PR_ProspectActivities.ActivityRecID = PR_Activities.InternalRecordIdentifier"
	...
	If owner>0 Then
		SQL8 = SQL8 &  " AND PR_Prospects.OwnerUserNo=" & owner
	End If

' Added this to ensure no duplicates
SQL8 = SQL8 & " AND (PR_ProspectStages.InternalrecordIdentifier IN (SELECT MAX(InternalrecordIdentifier) AS Expr1 "
SQL8 = SQL8 & " FROM PR_ProspectStages AS PR_ProspectStages_1 "
SQL8 = SQL8 & " GROUP BY ProspectRecID)) "

SQL8 = SQL8 & " AND (PR_ProspectActivities.InternalrecordIdentifier IN (SELECT MAX(InternalrecordIdentifier) AS Expr1 "
SQL8 = SQL8 & " FROM PR_ProspectActivities AS PR_ProspectActivities_1 "
SQL8 = SQL8 & " GROUP BY ProspectRecID)) "
	...
	SELECT CASE orderValue
		...
		CASE "1"
			SQL8=SQL8 & " ORDER BY Company " & orderType
		CASE "2"
			SQL8=SQL8 & " ORDER BY PR_Activities.Activity " & orderType
		CASE "3"
			SQL8=SQL8 & " ORDER BY ActivityDueDate " & orderType
		CASE "4"
		...
		CASE ELSE
			SQL8=SQL8 & " ORDER BY NextActivityDueDate  "
	END SELECT		

		'Response.write(SQL8 & "<br><br><br>")
		...
		'Response.write("RecsPerPage 1 : " & RecsPerPage & "<br>")
		
		'PgNum = Request.QueryString("PgNum")
		
		'totalProspectCount = TotalNumberOfProspects()
		
		'If NOT IsNumeric(RecsPerPage) OR RecsPerPage = "" Then RecsPerPage = 25
		'If NOT IsNumeric(PgNum) OR PgNum = "" Then PgNum = 1
		'If RecsPerPage = 0 Then RecsPerPage = 25
		...
		If not rs8.EOF Then
			...
			col_stage = ""
			If GetCRMPermissionLevel(Session("UserNo")) = "READONLY" Then 
				col_stage = "<a class='getProspectInfo' href='#'>"&GetStageByNum(GetProspectCurrentStageByProspectNumber(InternalRecordIdentifier)) &"</a>"  
			ElseIf (cInt(GetProspectOwnerNoByNumber(InternalRecordIdentifier)) = cInt(Session("UserNo")) AND GetCRMPermissionLevel(Session("userNo")) = "WRITEOWNED") Then 
				col_stage = "<a class='getProspectInfo' data-toggle='modal' data-show='true' href='#' data-stage-id='"&GetProspectCurrentStageByProspectNumber(InternalRecordIdentifier) &"' data-prospect-id='"&InternalRecordIdentifier&"' data-target='#myProspectingModalEditStage' data-tooltip='true' data-title='Edit Prospect Stage'>"& GetStageByNum(GetProspectCurrentStageByProspectNumber(InternalRecordIdentifier))&" &nbsp;&nbsp;<i class='fa fa-pencil-square fa-lg' aria-hidden='true'></i></a>" 
			ElseIf (cInt(GetProspectOwnerNoByNumber(InternalRecordIdentifier)) <> cInt(Session("UserNo")) AND GetCRMPermissionLevel(Session("userNo")) = "WRITEOWNED") Then 
				col_stage = "<a class='getProspectInfo' href='#'>"& GetStageByNum(GetProspectCurrentStageByProspectNumber(InternalRecordIdentifier)) &"</a>"  					
			ElseIf GetCRMPermissionLevel(Session("userNo")) = "READWRITE" Then 
				col_stage = "<a class='getProspectInfo' data-toggle='modal' data-show='true' href='#' data-stage-id='"& GetProspectCurrentStageByProspectNumber(rs8("Expr1")) &"' data-prospect-id='"& InternalRecordIdentifier &"' data-target='#myProspectingModalEditStage' data-tooltip='true' data-title='Edit Prospect Stage'>"&GetStageByNum(GetProspectCurrentStageByProspectNumber(InternalRecordIdentifier))&" &nbsp;&nbsp;<i class='fa fa-pencil-square fa-lg' aria-hidden='true'></i></a>"  
			End If 
			...
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_company"":""<a href='viewProspectDetail.asp?i="& InternalRecordIdentifier& "'>" & removeUnusualForJSON(Company) & """"
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_nextactivity"":""" & col_nextactivity & """" '2
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_nextactivitydate"":""" & col_nextactivityduedate & """" '3
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_address"":""" & removeUnusualForJSON(Street) & """"
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_city"":""" & removeUnusualForJSON(City) &""""
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_state"":""" & removeUnusualForJSON(State)  & """" '6
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_zip"":""" & removeUnusualForJSON(PostalCode) & """" '7
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_leadsource"":""" & removeUnusualForJSON(GetLeadSourceByNum(LeadSourceNumber))  & """" '8
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_stage"":""" & col_stage & """" '9
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_stagedate"":""" & col_stagedate  & """"
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_industry"":""" & removeUnusualForJSON(GetIndustryByNum(IndustryNumber))  & """" '11
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_numemployees"":""" & GetEmployeeRangeByNum(EmployeeRangeNumber)  & """"
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_owner"":""" & removeUnusualForJSON(GetUserDisplayNameByUserNo(OwnerUserNo))  & """" '13
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_createddate"":""" & Month(tmpCreatedDate) & "/" & Day(tmpCreatedDate) & "/" & Right(Year(tmpCreatedDate),2) & """"
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_createdby"":""" & GetUserDisplayNameByUserNo(CreatedByUserNo) & """" '15
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_telemarketer"":""" & removeUnusualForJSON(col_telemarketer) & """"
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_numpantries"":""" & NumberOfPantries & """"
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_prospectid"":""" & InternalRecordIdentifier & """"
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_watch"":""" & col_watch & """"
			JSONdata=JSONdata & ","
			JSONdata=JSONdata & """col_edit"":""<a href='viewProspectDetail.asp?i="&InternalRecordIdentifier&"'><p data-placement='top' data-toggle='tooltip' title='Edit'><button class='btn btn-success btn-xs' data-title='Edit' data-toggle='modal' data-target='#edit' ><span class='glyphicon glyphicon-pencil'></span></button></p></a>"""			
...
removeUnusualForJSON=REPLACE(Server.HTMLEncode(value),"""","&quot;")
