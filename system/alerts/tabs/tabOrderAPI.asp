 <% If MUV_READ("routingModuleOn") = "Enabled" Then %>
			<div role="tabpanel" class="tab-pane fade in" id="OrderAPIAlerts"> 
				<br>
				<div class="row">
				 	<div class="col-lg-12">
					 	<p>
							 <a href="addAlertOrderAPI.asp">
						    	<button type="button" class="btn btn-success">Add New Order API Alert</button>
							</a>
					 	</p>
				 	</div>
				</div>
				...
				<div class="table-responsive">
		            <table    class="table table-striped table-condensed table-hover sortable">
		              ...
		              <tbody>
						...
						SQL = "SELECT * FROM SC_Alerts Where AlertType='OrderAPIAlert' order by AlertName"
						...
						If not rs.EOF Then
					        ...
					        %>
								...
								<tr>
									<td>
										<a href='editAlertOrderAPI.asp?a=<%= rs.Fields("InternalAlertRecNumber") %>'><%= rs.Fields("AlertName") %></a></td>
										<td>
										<%
											Select Case rs.Fields("Condition")
												Case "Order Contains Driver Notes"
													Response.Write("When an Order is received containing Driver Notes")										
												Case "Order Contains Warehouse Notes"
													Response.Write("When an Order is received containing Warehouse Notes")
												Case "RA Contains Driver Notes"
													Response.Write("When an RA is received containing Driver Notes")
												Case "RA Contains Warehouse Notes"
													Response.Write("When an RA is received containing Warehouse Notes")
												Case "RA Contains RA Notes"
													Response.Write("When an RA is received containing RA Notes")
											End Select
										%>
										</td>		
										<td>
									...
									<td>
							   		...
							   		<td><a href='deleteAlertQues.asp?a=<%=rs.Fields("InternalAlertRecNumber")%>&tab=OrderAPIAlerts'><i class="fas fa-trash-alt"></i></a></td>
