...
<h1 class="page-header"><i class="fa fa-fw fa-exclamation"></i>Edit Order API Alert</h1>
...
<form method="POST" action="editAlertOrderAPI_submit.asp" name="frmEditAlert" id="frmEditAlert"  onsubmit="return checkform();">
	...
	<div class="row row-line">
		...
		<div class="col-lg-3">
			<select class="form-control when-line" name="selCond" id="selCond" onchange="cndChanged();">
				<option value="Order Contains Driver Notes" <% If Condition = "Order Contains Driver Notes" Then Response.Write(" selected ") %>>An Order is received containing Driver Notes</option>
				<option value="Order Contains Warehouse Notes" <% If Condition = "Order Contains Warehouse Notes" Then Response.Write(" selected ") %>>An Order is received containing Warehouse Notes</option>
				<option value="RA Contains Driver Notes" <% If Condition = "RA Contains Driver Notes" Then Response.Write(" selected ") %>>An RA is received containing Driver Notes</option>
				<option value="RA Contains Warehouse Notes" <% If Condition = "RA Contains Warehouse Notes" Then Response.Write(" selected ") %>>An RA is received containing Warehouse Notes</option>
				<option value="RA Contains RA Notes" <% If Condition = "RA Contains RA Notes" Then Response.Write(" selected ") %>>An RA is received containing RA Notes</option>
			</select>
		</div>
		<!-- eof when select !-->
	</div>
		...
		<div class="col-lg-10">
			...
			<div class="row row-line">
				<div class="col-lg-12 alertbutton">
					<p align="right"><a href="<%= BaseURL %>system/alerts/main.asp#OrderAPIAlerts">
