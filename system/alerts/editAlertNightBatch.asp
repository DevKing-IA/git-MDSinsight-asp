...
<h1 class="page-header"><i class="fa fa-fw fa-exclamation"></i>Edit Night Batch Alert</h1>
...
<form method="POST" action="editAlertNightBatch_submit.asp" name="frmEditAlert" id="frmEditAlert"  onsubmit="return checkform();">
	...
	<div class="row row-line">
		...
		<div class="col-lg-3">
			<select class="form-control when-line" name="selCond" id="selCond" onchange="cndChanged();">
				<option value="Start"<%If Ucase(Condition) = "START" Then Response.Write(" selected ")%>>Night Batch Starts</option>
				<option value="Finish"<%If Ucase(Condition) = "FINISH" Then Response.Write(" selected ")%>>Night Batch Finishes</option>
				<option value="Running"<%If Ucase(Condition) = "RUNNING" Then Response.Write(" selected ")%>>Night Batch has been running longer than</option>
				<option value="Log"<%If Ucase(Condition) = "LOG" Then Response.Write(" selected ")%>>Night Batch Log received</option>
			</select>
		</div>
		<!-- eof when select !-->
