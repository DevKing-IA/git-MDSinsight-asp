...
<%
'**********************************************************
'NEED A BASE WHERE CLAUSE SO WE CAN BUILD FILTERS UPON IT
'**********************************************************
WHERE_CLAUSE = "WHERE AuditElementOrEventName <> '' " 
...
End IF
WHERE_CLAUSE = WHERE_CLAUSE & " AND CONVERT(date,AuditEntryDateTime) >= '"&txtStartDate&"' AND CONVERT(date,AuditEntryDateTime) <= '"&txtEndDate&"' "
...
<div class="row row-data">	
	...
	<form method="post" action="AuditTrail_MultiDay.asp" name="frmMultiDayAuditTrail">
		...
		<div class="col-lg-3">
			<% If IgnoreLogin <> "on" Then %>
				<input type='checkbox' class='check' id='chkIgnoreLogin' name='chkIgnoreLogin' onclick="document.frmMultiDayAuditTrail.submit ();"> Ignore login/logout
			<%Else%>
				<input type='checkbox' class='check' id='chkIgnoreLogin' name='chkIgnoreLogin' onclick="document.frmMultiDayAuditTrail.submit ();" checked> Ignore login/logout
			<%End If%>
			
			&nbsp;&nbsp;
			
			<% If IgnoreSystem <> "on" Then %>
				<input type='checkbox' class='check' id='chkIgnoreSystem' name='chkIgnoreSystem' onclick="document.frmMultiDayAuditTrail.submit ();"> Ignore SYSTEM
			<%Else%>
				<input type='checkbox' class='check' id='chkIgnoreSystem' name='chkIgnoreSystem' onclick="document.frmMultiDayAuditTrail.submit ();" checked> Ignore SYSTEM
			<%End If%>
		</div>
