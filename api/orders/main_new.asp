...
$(document).ready(function() {
    ...
    $('#resendtoBackend').click( function () {

		OrderID = $('#txtInternalRecordIdentifier').val();
		
		$.ajax({
			type:"POST",
			url: "AjaxForOrderAPI.asp",
			data: "action=rePostOrderToBackend",
			    ...
			    'See if we allow reposting
			   		If GetAPIRepostURL() <> "" AND APIRepostOrders() = True Then 
				    	If GetUserOrderAPIPermissionLevel(Session("UserNo")) = "READ_RESEND" Or UserIsAdmin(Session("UserNo")) Then
							...
							<button type='button' class='btn btn-success btn-sm btn-block' data-toggle='modal' data-target='#InternalRecordIdentifier<%=rsOrderHeader("InternalRecordIdentifier")%>' >
							
							<%
							'Response.Write("<input type='hidden' id='txtInternalRecordIdentifier' name='txtInternalRecordIdentifier' value='" & rsOrderHeader("InternalRecordIdentifier") & "'>")
							Response.Write("<i class='fa fa-share'></i>&nbsp;Re-send to " & GetTerm("Backend") )
							Response.Write("</button>")
							%>

							<div class="modal fade" id="InternalRecordIdentifier<%=rsOrderHeader("InternalRecordIdentifier")%>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
							<!--#include file="postToBackendModal.asp"-->
							</div>
