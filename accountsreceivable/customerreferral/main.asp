...
<h1 class="page-header">Add / Edit Customer Referral</h1>

<div class="row">
 	<div class="col-lg-12">
	 	<p>
 <a href="addCustomerReferral.asp">
    	<button type="button" class="btn btn-success">Add New Customer Referral</button>
	</a>
	 	</p>
            ...
            <table    class="table table-striped table-condensed table-hover table-bordered sortable">
              <thead>
               ...
               <tbody class='searchable'>
				...
				SQL = "SELECT * FROM AR_CustomerReferral order by InternalRecordIdentifier"
			        ...
			        %>
						...
						<tr>
							<%If rscust.Fields("InternalRecordIdentifier") = 0 Then %>
								<td><%= rscust.Fields("ReferralName")%></td>
							<% Else %>
								<td><a href='editCustomerReferral.asp?i=<%= rscust.Fields("InternalRecordIdentifier")%>'><%= rscust.Fields("ReferralName")%></a></td>
							<% End If%>							
							
							<td><%= rscust.Fields("Description")%></td>
							<td><%= rscust.Fields("Description2")%></td>
							<%' Allow delete or display modal
							If rscust.Fields("InternalRecordIdentifier") <> 0 Then 								
								If NumberOfCustomersWithReferral(TypeCode) = 0 Then %>
									<td><a href='deleteCustomerRefQues.asp?i=<%=rscust.Fields("InternalRecordIdentifier")%>'><i class="fas fa-trash-alt"></i></a></td>
								<%Else %>
									<td><a data-toggle="modal" data-show="true" href="deleteCustomerRefModal.asp?i=<%= rscust.Fields("InternalRecordIdentifier") %>" data-target="#myModal"><i class="fas fa-trash-alt"></i></a></td>
								<% End If %>
							<%Else %>
								<td>&nbsp;</td>							
							<% End If %>
