...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#consolidatedInvoiceEmailModal').on('show.bs.modal', function(e) {
    		...
    		$modal.find('#myEmailConsolidatedInvoiceModalChainLabel').html("Email Unpaid Consolidated Invoice for Chain " + ChainID + " (" + ChainName + ") - Invoice #" + InvoiceNumber);
    		
	    	$.ajax({
				type:"POST",
				url: "../../../inc/InSightFuncs_AjaxForARAP.asp",
				cache: false,
				data: "action=GetContentForEmailConsolidatedInvoiceModalChain&consInvoiceNumber=" + encodeURIComponent(InvoiceNumber) + "&ChainID=" + encodeURIComponent(ChainID) + "&endDate=" + encodeURIComponent(EndDate) + "&paidOrUnpaid=UNPAID",
...
ImpVar = baseURL & "accountsreceivable/reports/consolidatedInvUnpaid/consolidated_stmt_frm_chn_PDFgen.asp?s=" & Replace(StartDate,"/","~") & "&e=" & Replace(EndDate,"/","~")& "&c=" & Account
...
%>	
<h1 class="page-header"><i class="fa fa-file-text"></i>  Consolidated Invoice Unpaid (Chain)</h1>
<!-- row !-->
