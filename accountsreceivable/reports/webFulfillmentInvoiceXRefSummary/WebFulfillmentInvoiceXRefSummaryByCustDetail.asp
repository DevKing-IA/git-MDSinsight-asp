...
CustClassCode = GetCustomerClassCodeByCustID(CustID) 
CustClassCodeName = GetCustomerClassNameByID(CustClassCode)

CreateAuditLogEntry "Report","Report","Minor",0, MUV_Read("DisplayName") & " ran the report: Web Fulfillment Invoice Cross Reference Customer Detail Cust ID: " & CustID
...
Response.Write("<br><br>Processing Web Fulfillment and Invoice Cross Reference Customer Detail Summary Data<br><br>This may take up to a full minute, please wait...<br><br>")
...
If OCSWebOrderOrMDSInvoice = "OCS" Then
	...
	If DatesOrPeriods = "Periods" Then
		...
		If cInt(PeriodStartYear) > 0 AND cInt(PeriodEndYear) > 0 Then
			...
			PageHeaderText = "" & CustName & ", Account " & CustID &  ", [" & CustClassCodeName & "]<br>Web/Invoice XREF Customer Detail For OCS Web Orders In Period " & PeriodStartNum & " of " & PeriodStartYear & " to Period " & PeriodEndNum & " of " & PeriodEndYear
		...
		Else
			...
			PageHeaderText = "" & CustName & ", Account " & CustID &  ", [" & CustClassCodeName & "]<br>Web/Invoice XREF Customer Detail For OCS Web Orders In Current Period "
	...
	ElseIf DatesOrPeriods = "Dates" Then
		...
		PageHeaderText = "" & CustName & ", Account " & CustID &  ", [" & CustClassCodeName & "]<br>Web/Invoice XREF Customer Detail For OCS Web Orders In Date Range"
...
ElseIf OCSWebOrderOrMDSInvoice = "MDS" Then
	...
	If DatesOrPeriods = "Periods" Then
		...
		If cInt(PeriodStartYear) > 0 AND cInt(PeriodEndYear) > 0 Then
			...
			PageHeaderText = "" & CustName & ", Account " & CustID &  ", [" & CustClassCodeName & "]<br>Web/Invoice XREF Customer Detail For MDS Invoices In Period " & PeriodStartNum & " of " & PeriodStartYear & " to Period " & PeriodEndNum & " of " & PeriodEndYear			
			PageHeaderText = PageHeaderText & " (" & FormatDateTime(PeriodStartDate,2) & " - " & FormatDateTime(PeriodEndDate,2) & ")&nbsp;&nbsp;"
		...
		Else
			...
			PageHeaderText = "" & CustName & ", Account " & CustID &  ", [" & CustClassCodeName & "]<br>Web/Invoice XREF Customer Detail For MDS Invoices In Current Period "
			PageHeaderText = PageHeaderText & " (" & FormatDateTime(currentPeriodBeginDateDefault,2) & " - " & FormatDateTime(currentPeriodEndDateDefault,2) & ")&nbsp;&nbsp;"
	...
	ElseIf DatesOrPeriods = "Dates" Then
		...
		PageHeaderText = "" & CustName & ", Account " & CustID &  ", [" & CustClassCodeName & "]<br>Web/Invoice XREF Customer Detail For MDS Invoices In Date Range"
		PageHeaderText = PageHeaderText & " (" & FormatDateTime(RangeStartDateCustomize,2) & " - " & FormatDateTime(RangeEndDateCustomize,2) & ")&nbsp;&nbsp;"
...
Else
	...
	PageHeaderText = "" & CustName & ", Account " & CustID &  ", [" & CustClassCodeName & "]<br>Web/Invoice XREF Detail for All Orders In Current Period "
...
SQLMasterWebFulfillment = SQLMasterWebFulfillment & " AND (CustID = '" & CustID & "') "
...
<h3 class="page-header">
	...
	<a href="<%= BaseURL %>accountsreceivable/reports/WebFulfillmentInvoiceXRefSummaryByCust.asp"><button type="button" class="btn btn-success"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back To All Customers</button></a><br><br>
