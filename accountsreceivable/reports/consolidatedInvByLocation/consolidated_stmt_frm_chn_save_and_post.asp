...
<%
	...
	'**************************************************************************************************
		...
		SQLInvoices = SQLInvoices & "(SELECT CustNum FROM AR_Customer WHERE ChainNum = " & ChainID & ") "
		...
		xmlData = xmlData & "<SUM_INVOICE_ID>" & Trim(ChainID) & Trim(Replace(EndDate,"/","")) & "</SUM_INVOICE_ID>"
		...
		xmlData = xmlData & "<CUST_ID></CUST_ID>"
		xmlData = xmlData & "<CHAIN_ID>" & Trim(ChainID) & "</CHAIN_ID>"
		...
		SQLInvoicesSingle = "SELECT * FROM " & MUV_Read("SQL_Owner") & ".InvoiceHistory WHERE "
		SQLInvoicesSingle = SQLInvoicesSingle & "IvsDate >= '" & StartDate & "' AND IvsDate <= '" & EndDate & "' "
		SQLInvoicesSingle = SQLInvoicesSingle & "AND CustNum IN "
		SQLInvoicesSingle = SQLInvoicesSingle & "(SELECT CustNum FROM AR_Customer WHERE ChainNum = " & ChainID & ") "
		...
		If not rsInvoicesSingle.Eof Then
			...
			Do While NOT rsInvoicesSingle.EOF
				...
				xmlData = xmlData & "<CUST_ID>" & rsInvoicesSingle("CustNum") & "</CUST_ID>"
