...
<h1 class="page-header">Add / Edit Customer Types</h1>

<div class="row">
 	<div class="col-lg-12">
	 	<p>
 <a href="addcustomerType.asp">
    	<button type="button" class="btn btn-success">Add New Customer Type</button>
	</a>
	 	</p>
            ...
            <table    class="table table-striped table-condensed table-hover table-bordered sortable">
              <thead>
               ...
               <tbody class='searchable'>
				...
				SQL = "SELECT * FROM AR_CustomerType order by InternalRecordIdentifier"
			        ...
			        %>
						...
						<tr>
							<%If rscust.Fields("InternalRecordIdentifier") = 0 Then %>
								<td><%= rscust.Fields("TypeDescription")%></td>
							<% Else %>
								<td><a href='editCustomerType.asp?i=<%= rscust.Fields("InternalRecordIdentifier")%>'><%= rscust.Fields("TypeDescription")%></a></td>
							<% End If%>							
							
							<%' Allow delete or display modal
							If rscust.Fields("InternalRecordIdentifier") <> 0 Then 								
								If NumberOfCustomersWithType(TypeCode) = 0 Then %>
									<td><a href='deleteCustomerTypeQues.asp?i=<%=rscust.Fields("InternalRecordIdentifier")%>'><i class="fas fa-trash-alt"></i></a></td>
								<%Else %>
									<td><a data-toggle="modal" data-show="true" href="deleteCustomerTypeModal.asp?i=<%= rscust.Fields("InternalRecordIdentifier") %>" data-target="#myModal"><i class="fas fa-trash-alt"></i></a></td>
								<% End If %>
							<%Else %>
								<td>&nbsp;</td>							
							<% End If %>
