...
<h1 class="page-header"><i class="fas fa-forklift"></i> <%= GetTerm("Inventory Control") %> Menu</h1>
...
<div class="container">
	...
	<% If MUV_Read("InventoryControlModuleOn") = "Enabled" Then %>
		...
		<% If userIsAdmin(Session("userNo")) = True OR userIsCSR(Session("userNo")) = True Then %>
			<div class="row">
				<div class="col-md-12">
					<div class="menu-option">
						<div class="menu-split menu-info"><i class="fa fa-arrow-right" aria-hidden="true"></i></div>
						<div class="menu-text"><a href="<%= BaseURL %>inventorycontrol/skumapping/main.asp">Add/Edit <%= GetTerm("Inventory Control") %> SKU Mapping</a></div>
					</div>
				</div>
			</div>
		<% End If %>
