...
<div class="driver-menu">		
	  <select class="segment-select" id="driverTicketMenu">
	      <option value="main_UnacknowledgedTickets.asp" selected="selected">UNACK (<%=NumberOfServiceTicketsAwaitingACKFromTech(Session("UserNo"))%>)</option>
...
Set rs = cnn8.Execute(SQL)
	...
	If not rs.EOF Then
						...
						<h6 class="mb-1 mt-1">Ticket #<%= MemoNumber %>
								...
								<% If CustHasServiceTicketNotes(MemoNumber) = True Then %>
									<% If NoteNewServiceTicketForUser(MemoNumber) = True Then %>
										<!-- Unread Note Icon-->
										<a href="viewServiceMemoNotes.asp?t=<%= MemoNumber %>&tab=unacknowledged">
								...
								<% Else %>
									<!-- Add New Note Icon -->
									<a href="addServiceMemoNote.asp?t=<%= MemoNumber %>&tab=unacknowledged">
										<span class="fa-stack" style="vertical-align: top;">
											<i class="fas fa-sticky-note fa-stack-2x" style="color:#28a745;"></i>
		...
		%></div><%
	Else
		%>You Have No Unacknowledged Service Calls.<%
	End IF
