...
<SCRIPT LANGUAGE="JavaScript">
    ...
    {
        if (document.frmswap.selAssetID.value != "") {
      	  if (document.frmswap.txtAssetTagNumber.value != "") {
        ...
        if (document.frmswap.selAssetID.value != "") {
      	  if (document.frmswap.txtAssetLocation.value == "") {
...
<h1 class="fieldservice-heading">
	...
	Swap Equipment Ticket #<%=SelectedMemoNumber%>
</h1>

<form action="swap_submit.asp" method="POST" ENCTYPE="multipart/form-data" name="frmswap" id="frmswap" onsubmit="return validateswapform();"> 
...
<div class="container-fluid">
		...
		<% If SelectedMemoNumber <> "" Then %>
			...
			<div id="accordion">
				  ...
				  <div class="card">
				    <div class="card-header" id="headingSwap">
				      <h5 class="mb-0">
				        <button class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#collapseSwap" aria-expanded="false" aria-controls="collapseTwo">
				          <i class="fas fa-random"></i> Swap Authorization
				         </button>
				      </h5>
				    </div>
				    <div id="collapseSwap" class="collapse" aria-labelledby="headingSwap" data-parent="#accordion">
				      <div class="card-body">
					        <select name="selPreAuth" id="selPreAuth" class="form-control input-lg">
								<option value="">Tap to select the person who authorized this swap</option>
								<option value="noneselected">-- Not Preauthorized --</option>
								<%	
								...
								SQL = "SELECT userNo,userDisplayName FROM " & MUV_Read("SQL_Owner") & ".tblUsers WHERE userCanAuthSwaps = 1 ORDER BY userDisplayName"
								
								set rs = cnn8.Execute (SQL)
								If not rs.EOF Then
						
									Do While Not rs.EOF
										tempUserno = rs("UserNo")
										tempDisplayName = rs("userDisplayName")
										
										strSelect1 =  "<option value='"& tempUserno &"'>"& tempDisplayName & "</option>"
						
										Response.Write(strSelect1)
										rs.MoveNext
									Loop
