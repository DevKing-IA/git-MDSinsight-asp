...
<%
	...
	CreateAuditLogEntry "Report","Report","Minor",0, MUV_Read("DisplayName") & " ran the report: Sales By Day Detail View"
	...
	If NOT rs.EOF Then
		...
		If IsNull(RangeStartDateCustomize) Then RangeStartDateCustomize = ""
		If IsNull(RangeEndDateCustomize) Then RangeEndDateCustomize = ""
	...
	<h3 class="page-header"><i class="fa fa-graduation-cap"></i> Sales By Day and Customer Class (Detailed View) For <%= PeriodBeingEvaluated %>
	&nbsp;&nbsp;
	...
	<% If SalesByDayReportTableSet(ReportNumber)Then %>
		<a href="<%= BaseURL %>bizintel/SalesByDayDetail_Customize_ClearValues.asp"><button type="button" class="btn btn-primary">Clear Customizations</button></a>
	<% End If %>
		<!-- eof modal button !-->
	</h3>
	
	<!--#include file="SalesByDayDetail_Customize.asp"-->	
			  ...
			  <thead>
			    ...
			    <tr>
					...
					For z = 0 to UBound(CustomerClassArray)
				        ...
				        <th>Total Cost $</th>
				        <th>Gross Profit %</th>
				        <th>Same Period Last Year Sales $</th>
				        <th>Same Period Last Year Cost $</th>
				        <th>Same Period Last Year Gross Profit %</th>
				        <th>+/- $ for Same Period Last Year</th>
		...
		Do While NOT rs.EOF
			...
			Do While CurrentDate = rs("ivsdate") 
				...
				If  CurrentDate = rs("ivsdate") Then
					...
					<% If rsGetSalesDataLastYear("TotSales") = 0 Then %>
						<td class="td-align zero"><%= FormatCurrency(rsGetSalesDataLastYear("TotSales"),2,-2,-1)%></td> 
					<% ElseIf rsGetSalesDataLastYear("TotSales") > 0 Then %>
						<td class="td-align positive"><%= FormatCurrency(rsGetSalesDataLastYear("TotSales"),2,-2,-1)%></td>
					<% ElseIf rsGetSalesDataLastYear("TotSales") < 0 Then %>
						<td class="td-align negative"><%= FormatCurrency(rsGetSalesDataLastYear("TotSales"),2,-2,-1)%></td>
					<% End If %>
					
					<% If rsGetSalesDataLastYear("TotCost") = 0 Then %>
						<td class="td-align zero"><%= FormatCurrency(rsGetSalesDataLastYear("TotCost"),2,-2,-1)%></td> 
					<% ElseIf rsGetSalesDataLastYear("TotCost") > 0 Then %>
						<td class="td-align positive"><%= FormatCurrency(rsGetSalesDataLastYear("TotCost"),2,-2,-1)%></td>
					<% ElseIf rsGetSalesDataLastYear("TotCost") < 0 Then %>
						<td class="td-align negative"><%= FormatCurrency(rsGetSalesDataLastYear("TotCost"),2,-2,-1)%></td>
					<% End If %>
					
					<% If SelectedPeriodLastYear_GrossProfitPct = 0 Then %>
						<td class="td-align zero"><%= FormatNumber(SelectedPeriodLastYear_GrossProfitPct,2) %>%</td> 
					<% ElseIf SelectedPeriodLastYear_GrossProfitPct > 0 Then %>
						<td class="td-align positive"><%= FormatNumber(SelectedPeriodLastYear_GrossProfitPct,2) %>%</td>
					<% ElseIf SelectedPeriodLastYear_GrossProfitPct < 0 Then %>
						<td class="td-align negative"><%= FormatNumber(SelectedPeriodLastYear_GrossProfitPct,2) %>%</td>
					<% End If %>
					
					
					<% If PlusMinusSamePeriodLastYearDlrs = 0 Then %>
						<td class="td-align zero"><%= FormatCurrency(PlusMinusSamePeriodLastYearDlrs,2)%></td> 
					<% ElseIf PlusMinusSamePeriodLastYearDlrs > 0 Then %>
						<td class="td-align positive"><%= FormatCurrency(PlusMinusSamePeriodLastYearDlrs,2)%></td>
					<% ElseIf PlusMinusSamePeriodLastYearDlrs < 0 Then %>
						<td class="td-align negative"><%= FormatCurrency(PlusMinusSamePeriodLastYearDlrs,2)%></td>
					<% End If %>
		...
		%>
		    ...
		    <tr>
				...
				For z = 0 to UBound(CustomerClassArray)
					...
					<% If TotalCostThisYear_GrandTotal = 0 Then %>
						<td class="td-align zero"><strong><%= FormatCurrency(TotalCostThisYear_GrandTotal,2,-2,-1)%></strong></td> 
					<% ElseIf TotalCostThisYear_GrandTotal > 0 Then %>
						<td class="td-align positive"><strong><%= FormatCurrency(TotalCostThisYear_GrandTotal,2,-2,-1)%></strong></td>
					<% ElseIf TotalCostThisYear_GrandTotal < 0 Then %>
						<td class="td-align negative"><strong><%= FormatCurrency(TotalCostThisYear_GrandTotal,2,-2,-1)%></strong></td>
					<% End If %>
					...
					<% End If %>
  					...
  					<% If TotalSalesLastYear_GrandTotal = 0 Then %>
						<td class="td-align zero"><strong><%= FormatCurrency(TotalSalesLastYear_GrandTotal,2,-2,-1)%></strong></td> 
					<% ElseIf TotalSalesLastYear_GrandTotal > 0 Then %>
						<td class="td-align positive"><strong><%= FormatCurrency(TotalSalesLastYear_GrandTotal,2,-2,-1)%></strong></td>
					<% ElseIf TotalSalesLastYear_GrandTotal < 0 Then %>
						<td class="td-align negative"><strong><%= FormatCurrency(TotalSalesLastYear_GrandTotal,2,-2,-1)%></strong></td>
					<% End If %>
					
					<% If TotalCostLastYear_GrandTotal = 0 Then %>
						<td class="td-align zero"><strong><%= FormatCurrency(TotalCostLastYear_GrandTotal,2,-2,-1)%></strong></td> 
					<% ElseIf TotalCostLastYear_GrandTotal > 0 Then %>
						<td class="td-align positive"><strong><%= FormatCurrency(TotalCostLastYear_GrandTotal,2,-2,-1)%></strong></td>
					<% ElseIf TotalCostLastYear_GrandTotal < 0 Then %>
						<td class="td-align negative"><strong><%= FormatCurrency(TotalCostLastYear_GrandTotal,2,-2,-1)%></strong></td>
					<% End If %>
         

					<% If GrossProfitLastYear_GrandTotal = 0 Then %>
						<td class="td-align zero"><strong><%= FormatNumber(GrossProfitLastYear_GrandTotal,2)%>%</strong></td> 
					<% ElseIf GrossProfitLastYear_GrandTotal > 0 Then %>
						<td class="td-align positive"><strong><%= FormatNumber(GrossProfitLastYear_GrandTotal,2)%>%</strong></td>
					<% ElseIf GrossProfitLastYear_GrandTotal < 0 Then %>
						<td class="td-align negative"><strong><%= FormatNumber(GrossProfitLastYear_GrandTotal,2)%>%</strong></td>
					<% End If %>
					

					<% If PlusMinusSamePeriodLastYearDlrs_GrandTotal = 0 Then %>
						<td class="td-align zero"><strong><%= FormatCurrency(PlusMinusSamePeriodLastYearDlrs_GrandTotal,2)%></strong></td> 
					<% ElseIf PlusMinusSamePeriodLastYearDlrs_GrandTotal > 0 Then %>
						<td class="td-align positive"><strong><%= FormatCurrency(PlusMinusSamePeriodLastYearDlrs_GrandTotal,2)%></strong></td>
					<% ElseIf PlusMinusSamePeriodLastYearDlrs_GrandTotal < 0 Then %>
						<td class="td-align negative"><strong><%= FormatCurrency(PlusMinusSamePeriodLastYearDlrs_GrandTotal,2)%></strong></td>
					<% End If %>
