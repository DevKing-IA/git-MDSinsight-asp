...
<%
	...
	Select Case MUV_READ("LOHVAR")
		Case "Secondary"
			...
			SQL = SQL & " AND  SecondarySalesman = " & Segment 
	...
	Set rs = cnn8.Execute(SQL)
		...
		Do While Not rs.EOF
			...
			If ShowThisRecord <> False Then
				...
				CurrentProductSales = GetCurrentPeriod_PostedProdSalesByCust(SelectedCustomerID) + GetCurrentPeriod_UnPostedProdSalesByCust(passedCustomerID)

				CurrentRentals =  GetCurrentPeriod_PostedRentalsByCust(SelectedCustomerID) + GetCurrentPeriod_UnPostedRentalsByCust(passedCustomerID)
				
				CurrentPeriodSales = CurrentProductSales + CurrentRentals 
				...
				If ShowThisRecord <> False Then
					...
					JSON=JSON+","
					JSON=JSON & """CurrentPSales"":""" & FormatCurrency(CurrentPeriodSales ,0,-2,0)  & """"
					JSON=JSON+","
