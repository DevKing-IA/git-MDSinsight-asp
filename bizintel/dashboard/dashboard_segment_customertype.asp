...
FilterCustType = Request.QueryString("p")
...
<%
CreateAuditLogEntry "Report","Report","Minor",0, MUV_Read("DisplayName") & " ran the report: Leakage Overview Single Customer Type"
...
Response.Write("<br><br>Creating Leakage Overview Single Customer Type<br><br>Please wait...<br><br>")
...
<h3 class="page-header">Leakage Overview for Customer Type <%=FilterCustType%> - <%=GetCustTypeByCode(FilterCustType)%> for Period <%=PeriodBeingEvaluated %>
&nbsp;&nbsp;
</h3>
...
SQL = SQL & " AND CustCatPeriodSales_ReportData.CustNum IN (SELECT CUSTNUM FROM AR_Customer WHERE CustType = '" & FilterCustType & "') "
...
'Response.Write(	LCPvs3PAvgSales &"x<br>")
				...
				If ShowThisRecord <> False Then
					...
					If LCPvs3PAvgSales > 0 Then
						Response.Write("<td align='" & ColumnAlign & "'  class='smaller-detail-line'>" & FormatCurrency(LCPvs3PAvgSales,0,-2,0) & "</td>")
					Else
						Response.Write("<td align='" & ColumnAlign & "'  class='smaller-detail-line negative'>" & FormatCurrency(LCPvs3PAvgSales,0,-2,0) & "</td>")
					End If
		   		   ' Response.Write("<td align='" & ColumnAlign & "'  class='smaller-detail-line'>" & FormatCurrency(LCPvs3PAvgSales,0) & "</td>")
				    If ShowPercentageColumns = True Then
		...
		'Response.Write("<tr><td>" & GrandTotLCPvs3PAvgSales & "</td></tr>")
