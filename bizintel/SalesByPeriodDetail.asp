...
<!--#include file="../inc/InSightFuncs_BizIntel.asp"-->
	...
	CreateAuditLogEntry "Report","Report","Minor",0, MUV_Read("DisplayName") & " ran the report: Sales By Period Detailed View"
	...
	If UBound(BasePeriodsRangeIntRecIDsArray) > 0 AND UBound(ComparisonPeriodsRangeIntRecIDsArray) > 0 Then
		...
		%>
			<h3 class="page-header"><i class="fa fa-graduation-cap"></i> Sales By Period and Customer Class (Detailed) For Periods
			<!-- modal button !-->
	...
	If UBound(BasePeriodsRangeIntRecIDsArray) <= 0 AND UBound(ComparisonPeriodsRangeIntRecIDsArray) <= 0 Then
		...
		%>
			<h3 class="page-header"><i class="fa fa-graduation-cap"></i> Sales By Period and Customer Class (Detailed) For <%= FormatDateTime(periodBeginDateDefault,2) %>-<%= FormatDateTime(periodEndDateDefault,2) %>
	...
	<!--#include file="SalesByPeriodDetail_Customize.asp"-->	
	...
	For y = 0 to UBound(BasePeriodsRangeIntRecIDsArray)
				...
				<table class="agenda">
				  <thead>
				    ...
				    <tr>
						...
						For z = 0 to UBound(CustomerClassArray)
							%>
						        <th>Base Period Total Sales $</th>
						        <th>Base Period Total Cost $</th>
						        <th>Base Period Gross Profit %</th>
						        <th>Comparison Period Sales $</th>
						        <th>Comparison Period Cost $</th>
						        <th>Comparison Period Gross Profit %</th>
						        <th>+/- $ for Comparison Period</th>
		...
		<% End If %>
			...
			For z = 0 to UBound(CustomerClassArray)	
				...
				<% If rsGetSalesDataComparePeriod("TotSales") = 0 Then %>
					<td class="td-align zero"><%= FormatCurrency(rsGetSalesDataComparePeriod("TotSales"),2,-2,-1)%></td> 
				<% ElseIf rsGetSalesDataComparePeriod("TotSales") > 0 Then %>
					<td class="td-align positive"><%= FormatCurrency(rsGetSalesDataComparePeriod("TotSales"),2,-2,-1)%></td>
				<% ElseIf rsGetSalesDataComparePeriod("TotSales") < 0 Then %>
					<td class="td-align negative"><%= FormatCurrency(rsGetSalesDataComparePeriod("TotSales"),2,-2,-1)%></td>
				<% End If %>
				
				<% If rsGetSalesDataComparePeriod("TotCost") = 0 Then %>
					<td class="td-align zero"><%= FormatCurrency(rsGetSalesDataComparePeriod("TotCost"),2,-2,-1)%></td> 
				<% ElseIf rsGetSalesDataComparePeriod("TotCost") > 0 Then %>
					<td class="td-align positive"><%= FormatCurrency(rsGetSalesDataComparePeriod("TotCost"),2,-2,-1)%></td>
				<% ElseIf rsGetSalesDataComparePeriod("TotCost") < 0 Then %>
					<td class="td-align negative"><%= FormatCurrency(rsGetSalesDataComparePeriod("TotCost"),2,-2,-1)%></td>
				<% End If %>
				
				<% If SelectedComparePeriod_GrossProfitPct = 0 Then %>
					<td class="td-align zero"><%= FormatNumber(SelectedComparePeriod_GrossProfitPct,2) %>%</td> 
				<% ElseIf SelectedComparePeriod_GrossProfitPct > 0 Then %>
					<td class="td-align positive"><%= FormatNumber(SelectedComparePeriod_GrossProfitPct,2) %>%</td>
				<% ElseIf SelectedComparePeriod_GrossProfitPct < 0 Then %>
					<td class="td-align negative"><%= FormatNumber(SelectedComparePeriod_GrossProfitPct,2) %>%</td>
				<% End If %>
				
				
				<% If PlusMinusComparePeriodDlrs = 0 Then %>
					<td class="td-align zero"><%= FormatCurrency(PlusMinusComparePeriodDlrs,2)%></td> 
				<% ElseIf PlusMinusComparePeriodDlrs > 0 Then %>
					<td class="td-align positive"><%= FormatCurrency(PlusMinusComparePeriodDlrs,2)%></td>
				<% ElseIf PlusMinusComparePeriodDlrs < 0 Then %>
					<td class="td-align negative"><%= FormatCurrency(PlusMinusComparePeriodDlrs,2)%></td>
				<% End If %>
		    ...
		    <tr>
			    ...
			    <tr>
					...
					'*******************************************************************
						...
						<% If TotalCostBasePeriod_GrandTotal = 0 Then %>
							<td class="td-align zero"><strong><%= FormatCurrency(TotalCostBasePeriod_GrandTotal,2,-2,-1)%></strong></td> 
						<% ElseIf TotalCostBasePeriod_GrandTotal > 0 Then %>
							<td class="td-align positive"><strong><%= FormatCurrency(TotalCostBasePeriod_GrandTotal,2,-2,-1)%></strong></td>
						<% ElseIf TotalCostBasePeriod_GrandTotal < 0 Then %>
							<td class="td-align negative"><strong><%= FormatCurrency(TotalCostBasePeriod_GrandTotal,2,-2,-1)%></strong></td>
						<% End If %>
						...
						<% End If %>
	  					...
	  					<% If TotalSalesComparePeriod_GrandTotal = 0 Then %>
							<td class="td-align zero"><strong><%= FormatCurrency(TotalSalesComparePeriod_GrandTotal,2,-2,-1)%></strong></td> 
						<% ElseIf TotalSalesComparePeriod_GrandTotal > 0 Then %>
							<td class="td-align positive"><strong><%= FormatCurrency(TotalSalesComparePeriod_GrandTotal,2,-2,-1)%></strong></td>
						<% ElseIf TotalSalesComparePeriod_GrandTotal < 0 Then %>
							<td class="td-align negative"><strong><%= FormatCurrency(TotalSalesComparePeriod_GrandTotal,2,-2,-1)%></strong></td>
						<% End If %>
							
						<% If TotalCostComparePeriod_GrandTotal = 0 Then %>
							<td class="td-align zero"><strong><%= FormatCurrency(TotalCostComparePeriod_GrandTotal,2,-2,-1)%></strong></td> 
						<% ElseIf TotalCostComparePeriod_GrandTotal > 0 Then %>
							<td class="td-align positive"><strong><%= FormatCurrency(TotalCostComparePeriod_GrandTotal,2,-2,-1)%></strong></td>
						<% ElseIf TotalCostComparePeriod_GrandTotal < 0 Then %>
							<td class="td-align negative"><strong><%= FormatCurrency(TotalCostComparePeriod_GrandTotal,2,-2,-1)%></strong></td>
						<% End If %>
						
						<% If GrossProfitComparePeriod_GrandTotal = 0 Then %>
							<td class="td-align zero"><strong><%= FormatNumber(GrossProfitComparePeriod_GrandTotal,2)%>%</strong></td> 
						<% ElseIf GrossProfitComparePeriod_GrandTotal > 0 Then %>
							<td class="td-align positive"><strong><%= FormatNumber(GrossProfitComparePeriod_GrandTotal,2)%>%</strong></td>
						<% ElseIf GrossProfitComparePeriod_GrandTotal < 0 Then %>
							<td class="td-align negative"><strong><%= FormatNumber(GrossProfitComparePeriod_GrandTotal,2)%>%</strong></td>
						<% End If %>
						
						<% If PlusMinusComparePeriodDlrs_GrandTotal = 0 Then %>
							<td class="td-align zero"><strong><%= FormatCurrency(PlusMinusComparePeriodDlrs_GrandTotal,2)%></strong></td> 
						<% ElseIf PlusMinusComparePeriodDlrs_GrandTotal > 0 Then %>
							<td class="td-align positive"><strong><%= FormatCurrency(PlusMinusComparePeriodDlrs_GrandTotal,2)%></strong></td>
						<% ElseIf PlusMinusComparePeriodDlrs_GrandTotal < 0 Then %>
							<td class="td-align negative"><strong><%= FormatCurrency(PlusMinusComparePeriodDlrs_GrandTotal,2)%></strong></td>
						<% End If %>
