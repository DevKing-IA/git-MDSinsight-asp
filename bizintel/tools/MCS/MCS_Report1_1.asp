...
$(document).ready(function() {
	...
	$('#tableSuperSum').DataTable({
			...
			columnDefs: [
			        ...
			        { type: 'currency', targets: 10}
			    ]	        
	    }
	);

	if($('#number_entire').val()){
		console.log($('#number_entire').val());
		$('#tableSuperSum_info').text("Showing 1 to "+$('#number_entire').val()+" of "+$('#number_entire').val()+" entries" ); 
		$('#mcs_tab').text("Chains("+$('#number_entire').val()+")"); 
	}

	var childtable = $('#tableSuperSumClients').DataTable({
	        scrollY: 500,
	        scrollCollapse: true,
	        paging: false,
	        order: [[ 13, 'desc' ]],
	...
	if(childtable.rows().count()){
		$('#clients_tab').text("Clients("+childtable.rows().count()+")"); 
	}
	//$("#tableSuperSum_info").hide();
	$('#clients_tab').on("click", function(){
...
<ul class="nav nav-tabs">
	<li class="active">
        <a id="clients_tab" href="#clients" data-toggle="tab">Clients(0)</a>
	</li>
	<li>
        <a id="mcs_tab" href="#mcs" data-toggle="tab">Chains(0)</a>
	</li>
	<li>
		<a id="pendingcharges_tab" href="#pendingcharges" data-toggle="tab">Pending Charges</a>
	</li>
</ul>

<div class="tab-content">
	<div class="tab-pane active" id="clients">
		<!--#include file="MCS_Report1_tab_clients.asp"-->
	</div>
	<div class="tab-pane" id="mcs">
		<!--#include file="MCS_Report1_tab_chains.asp"-->
	</div>
