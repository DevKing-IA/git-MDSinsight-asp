...
class AdminController
{
    ...
    public function resetPassword() {
        ...
        try {
            Mail::send(['text' => 'mail'], $data, function ($message) use ($email) {
                ...
                ('Recovery Password');
                $message->from('portal@mydomain.tld', 'Web Portal');
            });
        } catch (Exception $e) {
    ...
    public function checkDomain() {
        ...
        if (Domain::where('id', $id)->count() > 0) {
            ...
            foreach ($hosts as $host) {
                if (strpos($host, 'myproduct') !== false) {
                    $is_active = 1;
                    break;
                }
            }
    ...
    public function showSearchPage() {
        ...
        else {$date_from = date("Y-m-d");$search_clause[] = ['timestamp', '>=', "$date_from"];}

        if (isset($date_to))
            $search_clause[] = ['timestamp', '<=', "$date_to"." 23:59:59"];
        else {$date_to = date("Y-m-d");$search_clause[] = ['timestamp', '<=', "$date_to"." 23:59:59"];};
    ...
    function saveBlackAndWhitelistToFile() {
        ...
        $myfile = fopen("aha.txt", "w") or die("Unable to open file!");
    ...
    function saveWhitelistRcptToFile() {
        ...
        $myfile = fopen("doh.txt", "w") or die("Unable to open file!");
        fwrite($myfile, $content);
        fclose($myfile);
    }
