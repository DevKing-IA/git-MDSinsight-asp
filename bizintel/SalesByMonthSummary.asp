...
<!--#include file="../inc/InSightFuncs_BizIntel.asp"-->
	...
	If UBound(MonthYearCombinationsArray) >= 0 Then
		...
		%>
			...
			<% If SalesByDayReportTableSet(ReportNumber)Then %>
				<a href="<%= BaseURL %>bizintel/SalesByMonthSummary_Customize_ClearValues.asp"><button type="button" class="btn btn-primary">Clear Customizations</button></a>
			<% End If %>
				<!-- eof modal button !-->
							<br><br>
				<%= pageHeaderTextBaseVsCompare %>
	...
	<!--#include file="SalesByMonthSummary_Customize.asp"-->	
	...
	For y = 0 to UBound(MonthYearCombinationsArray)
		...
		<% If y = 0 Then %>			
			...
			For z = 0 to UBound(CustomerClassArray)	
				...
				'***************************************************************************************************
				'Build SQL STMT To Get Periods Sales Data For The BASE Month Date Range
