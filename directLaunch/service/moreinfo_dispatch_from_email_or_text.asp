	...
	If not rs.EOF Then
						...
						<div class="col-lg-12 mob-col">
							...
							<% If FS_TechCanDecline() Then %>
								<a href="Ack_dispatch_from_email_or_text.asp?t=<%=ServiceTicketNumber%>&c=<%=CustNum %>&u=<%=UserNumber %>&cl=<%=ClientKey %>" class="btn btn-primary btn-block btn-spacing pull-right" role="button">Tap To<br>Acknowledge</a>
								<a href="Rej_dispatch_from_email_or_text.asp?t=<%=ServiceTicketNumber%>&c=<%=CustNum %>&u=<%=UserNumber %>&cl=<%=ClientKey %>" class="btn btn-danger btn-block btn-spacing col-xs-6" role="button" >Tap To<br>Decline</a>
							<% Else %>
								<center>
									<a href="Ack_dispatch_from_email_or_text.asp?t=<%=ServiceTicketNumber%>&c=<%=CustNum %>&u=<%=UserNumber %>&cl=<%=ClientKey %>" class="btn btn-primary btn-block btn-spacing" role="button">Tap To<br>Acknowledge</a>
								</center>								
							<% End If %>
