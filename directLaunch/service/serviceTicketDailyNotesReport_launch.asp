...
'Self contained page
'Usage = "http://{xxx}.{domain}.com/directLaunch/service/serviceTicketDailyNotesReport_launch.asp?runlevel=run_now
	...
	Do While Not TopRecordset.EOF
		...
		If TopRecordset.Fields("ServiceModule") = "Enabled" Then
			...
			or (Instr(ucase(Request.ServerVariables("SERVER_NAME")),"DEV") <> 0 AND Instr(ucase(ClientKey),"D") <> 0) Then 
				...
				'**********************************************************
				' Now see if the service ticket notes report is on or off
				'**********************************************************				
				...
				If FieldServiceNotesReportOnOff = 1 Then
					...
					CreateAuditLogEntry "Field Service Notes Report Launch","Field Service Notes Report Launch","Minor",0,"Field Service Notes Report Launch ran."					
					...
					If MUV_READ("cnnStatus") = "OK" AND Session("MAILOFF") = 0 Then ' else it loops
						...
						Response.Write("<br>" & baseURL & "directlaunch/service/serviceTicketDailyNotesReport.asp?c=" & ClientKey & ",scale=0.8; hyperlinks=true; drawbackground=true<br>")
						Doc.ImportFromUrl baseURL & "directlaunch/service/serviceTicketDailyNotesReport.asp?c=" & ClientKey , "scale=0.8; hyperlinks=true; drawbackground=true; landscape=true"
						Response.Write(baseURL  & "directlaunch/service/serviceTicketDailyNotesReport.asp?c=" & ClientKey & "<br>")
						
						fn = "\clientfiles\" & trim(ClientKey) &"\z_pdfs\" & formatDateTime(Now(),2) & "-" & formatdatetime(Now(),4) & "_serviceTicketDailyNotesReport.pdf"
						fn = Replace(fn,"/","-")
						fn = Replace(fn,":","-")
						...
						Set rs_Settings_FieldService = cnn_Settings_FieldService.Execute(SQL_Settings_FieldService)
						If not rs_Settings_FieldService.EOF Then
							FieldServiceNotesReportUserNos = rs_Settings_FieldService("FieldServiceNotesReportUserNos")
							FieldServiceNotesReportAdditionalEmails = rs_Settings_FieldService("FieldServiceNotesReportAdditionalEmails")
							FieldServiceNotesReportEmailSubject = rs_Settings_FieldService("FieldServiceNotesReportEmailSubject")
						End If
						Set rs_Settings_FieldService = Nothing
						cnn_Settings_FieldService.Close
						Set cnn_Settings_FieldService = Nothing
						...
						For x = 0 to Ubound(Send_To_Array) -1
							...
							If FieldServiceNotesReportEmailSubject = "" Then
								emailSubject = "Field Service Notes Report (" & ClientKey & ")"
							Else
						 		emailSubject = FieldServiceNotesReportEmailSubject & " (" & ClientKey & ")"
							End If
							...
							emailBody = "Your Field Service Notes Report is attached. (" & ClientKey & ")"
							...
							SendMailWAtt "mailsender@" & maildomain,Send_To,emailSubject,emailBody,fnAttachmentArray,"Service","Field Service Notes Reportt","MDS Insight"
							
							CreateAuditLogEntry "Automated Field Service Notes Report","Automated Field Service Notes Report","Minor",0,"Automated Field Service Notes Report Sent to " & Send_To 
...
Sub WriteResponse(passedLogEntry)
	...
	SQL = SQL & ",'/directlaunch/service/serviceTicketDailyNotesReport_launch.asp'"
