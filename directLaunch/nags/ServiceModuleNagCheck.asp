	...
	Do While Not TopRecordset.EOF
		...
		If TopRecordset.Fields("routingModule") = "Enabled" Then
			...
			or (Instr(ucase(Request.ServerVariables("SERVER_NAME")),"DEV") <> 0 AND Instr(ucase(ClientKey),"D") <> 0) Then 
				...
				'**************************************************************
				'First see if the table exists & build it if need be
				...
				If MUV_READ("cnnStatus") = "OK" Then ' else it loops
					...
					SQLCalendar = "SELECT * FROM Settings_CompanyCalendar Where "
					SQLCalendar = SQLCalendar & "YearNum = " &  Year(Now()) & " AND "
					SQLCalendar = SQLCalendar & "MonthNum = " &  Month(Now()) & " AND "
					SQLCalendar = SQLCalendar & "DayNum = " &  Day(Now())
							
					Set rsCalendar = Server.CreateObject("ADODB.Recordset")
					rsCalendar.CursorLocation = 3 
					Set rsCalendar = cnnAlerts.Execute(SQLCalendar)
				
					' Are they Closed or Closing early today	
					If not rsCalendar.EOF Then
						If rsCalendar("OpenClosedCloseEarly") = "Closed" Then 
							WriteResponse ("Skipping, closed today.<br>")
							PROCESS_NAGS = False
						End If
						If rsCalendar("OpenClosedCloseEarly") = "Close Early" Then
							If cDate(TZTime()) >= cDate(rsCalendar("ClosingTime"))  Then
								WriteResponse ("Skipping, closed early today. Closed at " & FormatDateTime(rsCalendar("ClosingTime"),3) & " Time now is " & TZTime & "<br>")
								PROCESS_NAGS = False
							End If
						End If
					End If
