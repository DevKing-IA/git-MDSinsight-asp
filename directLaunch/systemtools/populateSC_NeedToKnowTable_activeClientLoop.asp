...
'Usage = "http://{xxx}.{domain}.com/directLaunch/systemtools/populateSC_NeedToKnowTable_activeClientLoop.asp?runlevel=run_now"
...
If Not TopRecordset.Eof Then
	...
	Do While Not TopRecordset.EOF
		...
		' This block just gives us a debug message
		...
		or (Instr(ucase(Request.ServerVariables("SERVER_NAME")),"FL") <> 0 AND Instr(ucase(ClientKey),"D") <> 0) Then 
			' Rem GO
		Else
			Response.Write("<br><br><br>******** SKIPPING " & ClientKey  & " not handled by this URL************<br><br>")
		End If
		...
		If MUV_READ("cnnStatus") = "OK" Then ' else it loops and excludes dev client keys
			...
			%><!--#include file="needtoknow/needtoknow_IC_Product_Bins.asp"--><%
			%><!--#include file="needtoknow/needtoknow_IC_Product_UPCcodes.asp"--><%
			%><!--#include file="needtoknow/needtoknow_Settings_CompanyCalendar.asp"--><%
			%><!--#include file="needtoknow/needtoknow_GlobalSettings_CustomerLogos.asp"--><%
			%><!--#include file="needtoknow/needtoknow_EQ_Equipment_AssetTagPrefix.asp"--><%		
			%><!--#include file="needtoknow/needtoknow_EQ_Equipment_Undefined.asp"--><%
			%><!--#include file="needtoknow/needtoknow_EQ_Equipment_ZeroDollarRentals.asp"--><%
