...
<%
UsageMessage = "http://www.mdsinsight.com/directLaunch/kiosks/service/FieldServiceKioskNoPaging.asp?pp={Your Passphrase}&cl={Your Client ID}&ri={Refresh Interval In Seconds}"
...
'Response.Write("Ubound Array: " & Ubound(RegionArray))
'Response.Write("<br>CurrentRegionToShow: " & Session("CurrentRegionToShow"))
'*****************************************************************************************
...
Else
	...
	If Recordset.Fields("routingModule") = "Enabled" Then 
		dummy = MUV_Write("routingModule",1)
	Else
		dummy = MUV_Write("routingModule",0)
	End If
...
%>
  <head>
	...
	If Session("RegionsToShow") <> "" And CurrentRegionToShowName <> "" then
		FSBoardKioskGlobalTitleText = "Region: " & CurrentRegionToShowName
	Else
		If FSBoardKioskGlobalTitleText = "" Then
			FSBoardKioskGlobalTitleText = "Today's Service Calls: " & WeekDayName(Datepart("w",Now())) & ", " & FormatDateTime(Now(),2)
		End If
	End If
    ...
    <script>
	   ...
	   $(document).ready(function(){		
			...
			//*************************************************************************************************************
			//*************************************************************************************************************
			//SPECIAL CODE FOR LINKED KIOSKS - LOOKS AT AJAX FUNCS FOR NEXT KIOSK TO DISPLAY
			//*************************************************************************************************************
			...
			var pagetimer = new Timer(function() {
			 ...
			 if ('<%= Session("RegionsToShow") %>' !== '') {
			 	...
			 	var regionsToDisplay = getUrlParameter('rgn');
			 	var currentRegionToDisplay = getUrlParameter('rgnc');
			 	
			 	
		 		//********************************************************************************
		 		//SEE IF REGIONS TO LOOP THROUGH, CHECK TO SEE IF WE HAVE VIEWED ALL THE REGIONS
				//*******************************************************************************

			 	if (regionsToDisplay !== '') {
			 	
			 		var regionsToDisplayArray = regionsToDisplay.split(',');
			 		var lastRegionToDisplay = regionsToDisplayArray[regionsToDisplayArray.length-1];
			 		
			 		//alert("regionsToDisplayArray: " + regionsToDisplay + " lastRegionToDisplay: " +lastRegionToDisplay + "  currentRegionToDisplay: " + currentRegionToDisplay);
			 		
			 		if (parseInt(lastRegionToDisplay) == parseInt(currentRegionToDisplay)) {
			 		
					 	$.ajax({
							type:"POST",
							url: "../../../inc/InSightFuncs_AjaxForServiceModals.asp",
							cache: false,
							data: "action=GetNextPageForLinkedKiosks&url=" + encodeURIComponent(url),
							success: function(urlToGoTo)
							 {
							 	//alert(urlToGoTo);
							 	if (urlToGoTo !== '') {
			 			...
			 			window.location = "../../kiosks/service/FieldServiceKioskNoPaging_linked_version.asp?pp=<%=Session("PassPhrase")%>&cl=<%=Session("ClientKey")%>&ri=<%=Session("RefreshInterval")%>&rgn=<%=Session("RegionsToShow")%>&rgnc=<%=Session("CurrentRegionToShow")%>";
			 		}
			 	}
	 		...
	 		//********************************************************************************
	 		//ELSE NO REGIONS TO LOOP THROUGH, USE STANDARD AJAX CHECK FOR NEXT KIOSK
			//*******************************************************************************
			...
			$('#serviceBoardTicketOptionsModal').on('show.bs.modal', function(e) {
		    	...
		    	$.ajax({
					...
					url: "../../../inc/InSightFuncs_AjaxForServiceModals.asp",
					cache: false,
					data: "action=GetContentForServiceBoardTicketOptionsModal&returnURL=directLaunch/kiosks/service/fieldservicekiosknopaging.asp&memoNum=" + encodeURIComponent(myTicketNumber) + "&custID=" + encodeURIComponent(myCustID) + "&userNo=" + encodeURIComponent(myUserNo),
			...
			$('#serviceBoardXferModal').on('show.bs.modal', function(e) {
		    	...
		    	$.ajax({
					...
					url: "../inc/../../../inc/InSightFuncs_AjaxForServiceModals.asp",
					cache: false,
					data: "action=GetContentForServiceBoardTransferRedispatchModal&returnURL=directLaunch/kiosks/service/fieldservicekiosknopaging.asp&memoNum=" + encodeURIComponent(myTicketNumber) + "&custID=" + encodeURIComponent(myCustID) + "&userNo=" + encodeURIComponent(myUserNo),
					success: function(response)
					 {
		...
		<table align="center">
			<tr>
		    	<td>
					...
					Set rs_FSBoardSumForAllRegions = Server.CreateObject("ADODB.Recordset")
					...
					If not rs_FSBoardSum.EOF Then
						...
						'**************************************************************************************************					
						'SQL STMT FOR REGIONAL KIOSK BOARD LOOP
						'**************************************************************************************************
						...
						If Session("RegionsToShow") <> "" Then
							...
							Else
							'**************************************************************************************************					
							'SQL STMT FOR ALL TRUCKS KIOSK BOARD LOOP
							'**************************************************************************************************
								...
								Do While Not rs_FSBoardSum.Eof
									...
									If userIsArchived(rs_FSBoardSum("UserNoOfServiceTech")) = False AND userIsEnabled(rs_FSBoardSum("UserNoOfServiceTech")) = True Then	
										...
										TechHasANYTicketsRegardlessOfRegion = False
								
										SQL_FSBoardSumForAllRegions = "SELECT DISTINCT CUstNum AS AccountNumber, MemoNumber FROM FS_ServiceMemosDetail "
										SQL_FSBoardSumForAllRegions = SQL_FSBoardSumForAllRegions & "WHERE MemoNumber IN "
					                    SQL_FSBoardSumForAllRegions = SQL_FSBoardSumForAllRegions & "(SELECT MemoNumber FROM FS_ServiceMemos WHERE "
					                    SQL_FSBoardSumForAllRegions = SQL_FSBoardSumForAllRegions & "(CurrentStatus = 'OPEN') OR "
					                    SQL_FSBoardSumForAllRegions = SQL_FSBoardSumForAllRegions & "(CurrentStatus = 'CLOSE' AND "
					                    
					                    SQL_FSBoardSumForAllRegions = SQL_FSBoardSumForAllRegions & "YEAR(RecordCreatedDateTime) = YEAR(GetDate()) AND "
					                    SQL_FSBoardSumForAllRegions = SQL_FSBoardSumForAllRegions & "MONTH(RecordCreatedDateTime) = MONTH(GetDate()) AND "
					                    SQL_FSBoardSumForAllRegions = SQL_FSBoardSumForAllRegions & "DAY(RecordCreatedDateTime) = DAY(GetDate()) "
					                    SQL_FSBoardSumForAllRegions = SQL_FSBoardSumForAllRegions & ")) AND UserNoOfServiceTech  = " & rs_FSBoardSum("UserNoOfServiceTech")
			
										'Response.Write(SQL_FSBoardSumForAllRegions & "<br>")
					                    
					                    Set rs_FSBoardSumForAllRegions = cnn_FSBoardSum.Execute(SQL_FSBoardSumForAllRegions)
					                    
										If not rs_FSBoardSumForAllRegions.EOF Then
										
											Do While NOT rs_FSBoardSumForAllRegions.EOF
											
												If GetServiceTicketCurrentStage(rs_FSBoardSumForAllRegions("MemoNumber")) <> "Received" Then 
											
													If LastTechUserNo(rs_FSBoardSumForAllRegions("MemoNumber")) = rs_FSBoardSum("UserNoOfServiceTech") Then
												
														TechHasANYTicketsRegardlessOfRegion = True
				
													End If
												
												End If
			
												If TechHasANYTicketsRegardlessOfRegion = True Then Exit Do
											
												rs_FSBoardSumForAllRegions.MoveNext
											Loop
										
										End IF
					                    
										'Only Write Route If The User Is Not Archived and Not Disabled
										If TechHasANYTicketsRegardlessOfRegion = True Then 
											Call TruckNumberWrite(rs_FSBoardSum("UserNoOfServiceTech")) 
										End IF
