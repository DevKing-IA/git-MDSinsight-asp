...
'*********************************************************************************************
'Whenever there are automated processes, etc, use user 0 to indicate it is a system process
'*********************************************************************************************
Session("UserNo") = 0
'*********************************************************************************************
...
ReturnURL = Request.Form("txtReturnURL")

Response.Write("maildomain:" & maildomain & "<br>")
...
If SendEmail="on" then
	If UserToDispatch <> 0 Then ' Normal xfer not UNDispatch
		If getUserEmailAddress(UserToDispatch) <> "" Then 
			...
			SendMail "mailsender@" & maildomain ,Send_To,emailSubject,emailBody,GetTerm("Service"),"Service Dispatch","INSIGHT"
		...
		Else
			...
			SendMail "mailsender@" & maildomain ,SEND_TO,"Unable to send dispatch email",emailBody,GetTerm("Service"),"Missing Email","INSIGHT"
	...
	If UserToDispatch = 0 Then ' This is an undispatch
		If getUserEmailAddress(HeldTech) <> "" Then 
			...
			If Instr(ucase(sURL),"DEV") <> 0 Then Send_To = "rsmith@ocsaccess.com"
			If Instr(ucase(sURL),"DEV2") <> 0 Then Send_To = "cgrecco@ocsaccess.com"
			SendMail "mailsender@" & maildomain ,Send_To,emailSubject,emailBody,GetTerm("Service"),"Cancel Dispatch","INSIGHT"
...
If SendText="on" then
	...
	If UserToDispatch <> 0 Then
		...
		Else
			...
			If Instr(ucase(sURL),"DEV") <> 0 Then SEND_TO = "rich@ocsaccess.com"
			SendMail "mailsender@" & maildomain ,SEND_TO,"Unable to send dispatch text message (" & MUV_READ("SERNO") & ")",emailBody,GetTerm("Service"),"Missing Email","INSIGHT"
	...
	If UserToDispatch <> 0 Then
		...
		Else
			...
			If Instr(ucase(sURL),"DEV") <> 0 Then SEND_TO = "rich@ocsaccess.com"
			SendMail "mailsender@" & maildomain ,SEND_TO,"Unable to send dispatch text message (" & MUV_READ("SERNO") & ")",emailBody,,GetTerm("Service"),"Missing Cell Number","INSIGHT"
...
Response.Redirect(BaseURL & "directLaunch/kiosks/service/FieldServiceKioskNoPaging.asp?pp=" & Session("PassPhrase") & "&cl=" & Session("ClientKey") & "&ri=" & Session("RefreshInterval"))
