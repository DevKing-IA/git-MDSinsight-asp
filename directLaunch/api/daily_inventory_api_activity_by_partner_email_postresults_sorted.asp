...
If Recordset.recordcount <= 0 then
	...
	%>MDS Insight: Unable to connect to SQL database. The server is not available or the credentials specified are incorrect. - Daily Inventory API Log By Partner Sorted By Result<%
	Response.End
Else
...
<head>
    ...
    <title>Daily Inventory API Log By Partner Sorted By Result</title>
	...
	Do While Not rsDailyAPIPartnersLoop.EOF
		...
		currentPartnerAPIKey = rsDailyAPIPartnersLoop("partnerAPIKey")
			...
			Response.Write("<hr>")
		   	...
		   	Response.Write("<h4 style='color: #3c763d; margin-top: 40px; font-size:23px;'>On Hand Adjustments For" & FormatDateTime(dateadd("d",-1,Date())) & "<h4><br>")			
			...
			Response.Write("<table style='margin-left:20px;width:1300px;'>")	
		        Response.Write("<thead>")
		            Response.Write("<tr style='border-bottom: 2px solid #ddd;'>")
		                ...
		                Response.Write("<th style='padding-top: 8px; text-align: right;' align='right' width='5%'>Result</th>")
		                Response.Write("<th style='padding-top: 8px; text-align: right;' align='right' width='4%'>Thread</th>")
		                Response.Write("<th style='padding-top: 8px; text-align: right;' align='right' width='52%'>Message (if any)</th>")
		                Response.Write("<th style='padding-top: 8px; text-align: right;' align='right' width='52%'>Comment (if any)</th>")
		            Response.Write("</tr>")
		        Response.Write("</thead>")
		        Response.Write("<tbody>")

				SQLInventoryAPI = "SELECT * FROM API_IC_PostResults "
	...
	'Response.Write(SQLInventoryAPI)
				...
				If NOT rsInventory.EOF Then
					...
					Do While Not rsInventory.EOF
						...
						PostResults = "" : PostStatusMessage = ""
						
						DailyCount = DailyCount + 1
						
						PostResults = rsInventory("PostResults")
						...
						Response.Write("<tr>")
							...
							Response.Write("<td style='padding-top: 8px; text-align: right;' align='left'>" &  rsInventory("RecordCreationDateTime") & "</td>")
			                Response.Write("<td style='padding-top: 8px; text-align: right;' align='left'>" & rsInventory("PostResults")& "</td>")
			                Response.Write("<td style='padding-top: 8px; text-align: right;' align='right'>" & rsInventory("Thread") & "</td>")
			             	Response.Write("<td style='padding-top: 8px; text-align: right;' align='left'>" & rsInventory("PostStatusMessage") & "</td>")
			             	Response.Write("<td style='padding-top: 8px; text-align: right;' align='left'>" & rsInventory("Comments") & "</td>")
			            Response.Write("</tr>")
...
Sub PageHeader
	...
	If PageNo > 1 Then Response.Write("<div style='page-break-before: always'>")
 	...
 	Response.Write("<img src='/clientfiles/" & ClientKey & "/logos/logo.png' style='float:left; margin-top:30px;'><center><h1 >DAILY INVENTORY API LOG <Br>BY PARTNER SORTED BY RESULT"  & "</h1><h2 class='generated' >Generated " & WeekDayName(WeekDay(DateValue(Now()))) & "&nbsp;" &  Now() & "</h2></center>")
