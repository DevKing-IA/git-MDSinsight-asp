...
'Self contained page
'Usage = "http://{xxx}.{domain}.com/directLaunch/needtoknowreports/inventory_needtoknow_report_launch.asp?runlevel=run_now
	...
	Do While Not TopRecordset.EOF
		...
		If TopRecordset.Fields("InventoryControlModule") = "Enabled" Then
			...
			or (Instr(ucase(Request.ServerVariables("SERVER_NAME")),"DEV") <> 0 AND Instr(ucase(ClientKey),"D") <> 0) Then 
				...
				'**********************************************************
				' Now see if the inventory need to know report is on or off
				'**********************************************************				
				...
				If MUV_READ("FORCE") <> "1" Then ' Force parameter tell it not to read the schedule
					...
					CheckSchedulerArray = Split(CheckScheduler("Settings_NeedToKnow","Schedule_InventoryNeedToKnowReportGeneration"),",")
					If CheckSchedulerArray(0) <> "1" Then
						N2KInventoryReportONOFF = 0 ' Just turn it off & let the page flow normally
					End If
					Response.Write("<b>CheckScheduler Results: " &  CheckSchedulerArray(1) & "&nbsp;&nbsp;(" & ClientKey  & ")</b><br>")
					CreateAuditLogEntry "Inventory Need2Know Report Launch","Inventory Need2Know Report Launch","Minor",0,"Inventory Need2Know Report Schedule check results: " & CheckSchedulerArray(1)
				End If
				'************************************
				...
				If N2KInventoryReportONOFF  = 1 Then
					...
					If MUV_READ("cnnStatus") = "OK" AND Session("MAILOFF") = 0 Then ' else it loops
						...
						'*********************************************************************
						'Now create and save INVENTORY NEED TO KNOW REPORT PDF
						'*********************************************************************
						...
						Response.Write("<br>" & baseURL & "directlaunch/needtoknowreports/inventory.asp?c=" & ClientKey & ",scale=0.8; hyperlinks=true; drawbackground=true<br>")
						Doc.ImportFromUrl baseURL & "directlaunch/needtoknowreports/inventory.asp?c=" & ClientKey, "scale=0.8; hyperlinks=true; drawbackground=true"
						Response.Write(baseURL  & "directlaunch/needtoknowreports/inventory.asp?c=" & ClientKey & "<br>")
						
						fn = "\clientfiles\" & trim(ClientKey) &"\z_pdfs\" & formatDateTime(Now(),2) & "-" & formatdatetime(Now(),4) & "_NeedToKnowReport_Inventory.pdf"
						fn = Replace(fn,"/","-")
						fn = Replace(fn,":","-")
						...
						If not rs_Settings_Global.EOF Then
							N2KInventoryEmailToUserNos = rs_Settings_Global("N2KInventoryEmailToUserNos")
							N2KInventoryUserNosToCC = rs_Settings_Global("N2KInventoryUserNosToCC")
							N2KInventoryEmailAddressesToCC = rs_Settings_Global("N2KInventoryEmailAddressesToCC")
						End If
						...
						'*******************************************************************************************************************************************************************
							...
							If N2KInventoryEmailAddressesToCC <> "" and not IsNull(N2KInventoryEmailAddressesToCC) Then
								tmpN2KInventoryEmailAddressesToCC   = trim(N2KInventoryEmailAddressesToCC)		
								If Len(tmpN2KInventoryEmailAddressesToCC) > 1 Then
									If Right(tmpN2KInventoryEmailAddressesToCC,1) <> ";" Then tmpN2KInventoryEmailAddressesToCC = tmpN2KInventoryEmailAddressesToCC & ";"
									Send_To = Send_To & tmpN2KInventoryEmailAddressesToCC
								End If	
							End If
							
							'Get user based emails
							If N2KInventoryEmailToUserNos <> "" Then
								UserNoList = Split(N2KInventoryEmailToUserNos,",")
							...
							If N2KInventoryEmailToUserNos <> "" Then
								UserNoList2 = Split(N2KInventoryUserNosToCC,",")
							...
							For x = 0 to Ubound(Send_To_Array) -1
								...
								emailSubject = "Insight Need To Know Report - Inventory (" & ClientKey & ")"
	
								emailBody = ""
								'Failsafe for dev
								sURL = Request.ServerVariables("SERVER_NAME")
								If Instr(ucase(sURL),"DEV.") <> 0 Then Send_To = "rich@ocsaccess.com"
								emailBody = "Your Insight Need To Know Report - Inventory is attached. (" & ClientKey & ")"
								...
								SendMailWMultipleAtt "mailsender@" & maildomain,Send_To,emailSubject,emailBody,fnAttachmentArray,GetTerm("Inventory"),"Need To Know","MDS Insight"
								
								CreateAuditLogEntry "Automated Need To Know Report - Inventory","Automated Need To Know Report - Inventory","Minor",0,"Automated Need To Know Report - Inventory Sent to " & Send_To 
		...
		Else
			...
			WriteResponse ("Skipping the client " & ClientKey & " because the Inventory need to know report is turned off.<BR>")
		
		End If ' for the report being turned off
			
		End If	
		
	Else ' is the inventory module enabled
		...
		WriteResponse ("Skipping the client " & ClientKey & " because the Inventory Module module is not enabled.<BR>")
		
	End If ' is the Inventory module enabled
...
Sub WriteResponse(passedLogEntry)
	...
	SQL = SQL & ",'Inventory Need To Know Report'"
	SQL = SQL & ",'/directlaunch/needtoknowreports/inventory_needtoknow_report_launch.asp'"
