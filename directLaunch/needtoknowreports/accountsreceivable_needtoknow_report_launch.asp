...
'Self contained page
'Usage = "http://{xxx}.{domain}.com/directLaunch/needtoknowreports/accountsreceivable_needtoknow_report_launch.asp?runlevel=run_now
	...
	Do While Not TopRecordset.EOF
		...
		If cInt(TopRecordset.Fields("arModule")) = 1 Then
			...
			or (Instr(ucase(Request.ServerVariables("SERVER_NAME")),"DEV") <> 0 AND Instr(ucase(ClientKey),"D") <> 0) Then 
				...
				'**********************************************************
				' Now see if the AR need to know report is on or off
				'**********************************************************				
				...
				If N2KARReportONOFF  = 1 Then
					...
					CreateAuditLogEntry "Need To Kow Report Launch","Need To Know Report Launch Report Launch","Minor",0,"Need To Know Report Launch Report Launch ran."					
					...
					If MUV_READ("cnnStatus") = "OK" AND Session("MAILOFF") = 0 Then ' else it loops
						...
						'*********************************************************************
						'Now create and save NEED TO KNOW REPORT PDF
						'*********************************************************************
						...
						Response.Write("<br>" & baseURL & "directlaunch/needtoknowreports/accountsreceivable.asp?c=" & ClientKey & ",scale=0.8; hyperlinks=true; drawbackground=true<br>")
						Doc.ImportFromUrl baseURL & "directlaunch/needtoknowreports/accountsreceivable.asp?c=" & ClientKey, "scale=0.8; hyperlinks=true; drawbackground=true"
						Response.Write(baseURL  & "directlaunch/needtoknowreports/accountsreceivable.asp?c=" & ClientKey & "<br>")
						
						fn = "\clientfiles\" & trim(ClientKey) &"\z_pdfs\" & formatDateTime(Now(),2) & "-" & formatdatetime(Now(),4) & "_NeedToKnowReport_AccountsReceivable.pdf"
						fn = Replace(fn,"/","-")
						fn = Replace(fn,":","-")
						...
						If not rs_Settings_Global.EOF Then
							N2KAREmailToUserNos = rs_Settings_Global("N2KAREmailToUserNos")
							N2KARUserNosToCC = rs_Settings_Global("N2KARUserNosToCC")
							N2KAREmailAddressesToCC = rs_Settings_Global("N2KAREmailAddressesToCC")
						End If
						...
						'*******************************************************************************************************************************************************************
							...
							If N2KAREmailAddressesToCC <> "" and not IsNull(N2KAREmailAddressesToCC) Then
								tmpN2KAREmailAddressesToCC   = trim(N2KAREmailAddressesToCC)		
								If Len(tmpN2KAREmailAddressesToCC) > 1 Then
									If Right(tmpN2KAREmailAddressesToCC,1) <> ";" Then tmpN2KAREmailAddressesToCC = tmpN2KAREmailAddressesToCC & ";"
									Send_To = Send_To & tmpN2KAREmailAddressesToCC
								End If	
							End If
							
							'Get user based emails
							If N2KAREmailToUserNos <> "" Then
								UserNoList = Split(N2KAREmailToUserNos,",")
							...
							If N2KAREmailToUserNos <> "" Then
								UserNoList2 = Split(N2KARUserNosToCC,",")
							...
							For x = 0 to Ubound(Send_To_Array) -1
								...
								emailSubject = "Insight Need To Know Report - Accounts Receivable (" & ClientKey & ")"
								...
								emailBody = "Your Insight Need To Know Report - Accounts Receivable is attached. (" & ClientKey & ")"
								...
								End If
								If (Instr(ucase(Request.ServerVariables("SERVER_NAME")),"DEV2")) <> 0 Then
									Send_To="cgrecco@ocsaccess.com"
								End If
															
								SendMailWMultipleAtt "mailsender@" & maildomain,Send_To,emailSubject,emailBody,fnAttachmentArray,GetTerm("Accounts Receivable"),"Need To Know","MDS Insight"
								
								CreateAuditLogEntry "Automated Need To Know Report - Accounts Receivable","Automated Need To Know Report - Accounts Receivable","Minor",0,"Automated Need To Know Report - Accounts Receivable Sent to " & Send_To 
		...
		Else
			...
			WriteResponse ("Skipping the client " & ClientKey & " because the Accounts Receivable need to know report is turned off.<BR>")
		
		End If ' for the report being turned off
			
		End If	
		
	Else ' is the Accounts Receivable module enabled
		...
		WriteResponse ("Skipping the client " & ClientKey & " because the Accounts Receivable Module module is not enabled.<BR>")
		
	End If ' is the Accounts Receivable module enabled
...
Sub WriteResponse(passedLogEntry)
	...
	SQL = SQL & ",'Accounts Receivable Need To Know Report'"
	SQL = SQL & ",'/directlaunch/needtoknowreports/accountsreceivable_needtoknow_report_launch.asp'"
