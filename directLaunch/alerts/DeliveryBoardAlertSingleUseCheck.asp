...
'Usage = "http://{xxx}.{domain}.com/directLaunch/alerts/DeliveryBoardAlerSingleUseCheck.asp?runlevel=run_now
...
If Not TopRecordset.Eof Then
	...
	Do While Not TopRecordset.EOF
		...
		If MUV_READ("cnnStatus") = "OK" Then ' else it loops
			...
			SQLAlerts = "SELECT * FROM SC_Alerts Where AlertType='DeliveryBoard'" 
			...
			If not rsAlert.EOF Then
				...
				Do While Not rsAlert.EOF
					...
					Response.Write("<b>Found Alert Named: " & rsAlert("AlertName") &"</b><br>") 
						...
						InvoiceNumberToCheck = rsAlert("ReferenceValue")
						...
						If rsAlert("Condition") = "This becomes the next stop" Then 
							...
							If GetCustNumberByInvoiceNumDelBoard(InvoiceNumberToCheck) = GetNextCustomerStopByTruck(GetTruckByInvoiceNumDelBoard(InvoiceNumberToCheck)) Then
								...
								'Send the alert the appropriate way
								'Apparently these alerts don't get deleted after they are sent
								'which might be ok in case a RESET is done, it would still stay in effect
								
								If AlertSent(rsAlert("InternalAlertRecNumber"),InvoiceNumberToCheck) <> True Then
								
									SendAlert rsAlert("InternalAlertRecNumber"),InvoiceNumberToCheck,rsAlert("Condition")
									
								Else
								
									Response.Write("<b>Alert Named: " & rsAlert("AlertName") &" was already sent...skipping...</b><br>") 			
...
Sub SendAlert (passedAlertNumber,PassedInvoiceNumber,AlertType)
	...
	If Send_To <> "" Then
		...
		Select Case AlertType
			Case "This becomes the next stop"
				emailSubject = "Invoice " & PassedInvoiceNumber & " set as Next Stop"
				emailHeadLineText = "Invoice # " & PassedInvoiceNumber & " has been set as the <i>Next Stop</i> by the driver as of " & FormatDateTime(GetLastDeliveryStatusChangeBYInvoiceNumDelBoard(PassedInvoiceNumber),3) & " today."
				txtMessage = "Invoice # " & PassedInvoiceNumber & " has been set as the Next Stop by the driver."
			Case "Stop is completed or skipped"
				If GetDeliveryStatusByInvoice(PassedInvoiceNumber) = "Delivered" Then
				...
				ElseIf GetDeliveryStatusByInvoice(PassedInvoiceNumber) = "No Delivery" Then
					emailSubject = "Invoice " & PassedInvoiceNumber & " not delivered"
					emailHeadLineText = "Invoice # " & PassedInvoiceNumber & " was updated to <i>not delivered</i> at " & FormatDateTime(GetLastDeliveryStatusChangeBYInvoiceNumDelBoard(PassedInvoiceNumber),3) & " today."
					txtMessage = "Invoice # " & PassedInvoiceNumber & " was updated to not delivered at " & FormatDateTime(Now(),3) & " today."
				End If
			Case "Delivered"
				...
				emailHeadLineText = "Invoice # " & PassedInvoiceNumber & " was updated to <i>delivered</i> at " & FormatDateTime(Now(),3) & " today."
			...
			Case "Partial"
				...
				emailHeadLineText = "The delivery for " & GetTerm("customer") & " " & GetCustNumberByInvoiceNumDelBoard(PassedInvoiceNumber) & " was <i>partially</i> updated at " & FormatDateTime(Now(),3) & " today."
