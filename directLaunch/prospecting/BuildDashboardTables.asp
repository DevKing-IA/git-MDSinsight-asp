		...
		If MUV_READ("cnnStatus") = "OK" Then 'AND Instr(Ucase(ClientKey),"D") = 0 Then ' else it loops and excludes dev client keys
			...
			If MUV_READ("ProspectingModule") = "Enabled" Then 
				...
				'******************************************
					...
					SQLBuildSQL  = "SELECT * FROM sysobjects Where Name= 'PR_DashboardDetailsUQ' AND xType= 'U'"
					...
					If rsBuildSQL.RecordCount = 0 Then 
						SkipThisClient = True
						Response.Write("<br>The table PR_DashboardDetailsUQ does not exist for " & ClientKey & " skipping this client<br>")
					Else
...
Response.Write("<b>**********NEW CODE FOR QUALIFIED TABLES</b><br><br>")
						...
						SQLBuildSQL = "DELETE FROM PR_DashboardDetailsQ"
						Set rsBuildSQL = cnnBuildSQL.Execute(SQLBuildSQL) 
						
						'Start by moving everything into the temp table
						SQLBuildSQL = "INSERT INTO PR_DashboardDetailsQ (ProspectRecID, OwnerUserNo, LeadSourceNumber, CreatedDate, CreatedByUserNo) "
						...
						'Now fill in all the stages
						SQLBuildSQL = "SELECT ProspectRecId FROM PR_DashboardDetailsQ ORDER BY ProspectRecId"
						Set rsBuildSQL = cnnBuildSQL.Execute(SQLBuildSQL) 
						...
						If Not rsBuildSQL.EOF Then
							...
							Do While NOT rsBuildSQL.EOF
								...
								If Not rsStages.EOF Then
									...
									SQLBuildSQL2 = "UPDATE PR_DashboardDetailsQ Set LastStageNumber = " & rsStages("StageRecID")  
									SQLBuildSQL2 = SQLBuildSQL2 & " ,LastStageIntRecID = " & rsStages("InternalRecordIdentifier") 
									SQLBuildSQL2 = SQLBuildSQL2 & " WHERE ProspectRecID = " & rsBuildSQL("ProspectRecID")
						...
						SQLBuildSQL2 = "DELETE FROM PR_DashboardDetailsQ WHERE LastStageNumber = 2 OR LastStageNumber = 1"
						Set rsBuildSQL2 = cnnBuildSQL.Execute(SQLBuildSQL2)
					
						
						'Now work on the summary table
						SQLBuildSQL = "DELETE FROM PR_DashboardSummaryByOwnerQ"
						Set rsBuildSQL = cnnBuildSQL.Execute(SQLBuildSQL) 
						...
						rsAllOwners.CursorLocation = 3 
						rsAllOwners.CursorType = 3
						SQLAllOwners = "SELECT DISTINCT OwnerUserNo,* FROM PR_DashboardDetailsQ"
						Set rsAllOwners = cnnBuildSQL3.Execute(SQLAllOwners) 
						...
						If NOT rsAllOwners.EOF Then		
							Do While Not rsAllOwners.EOF
									...
									SQLSummary1 = "INSERT INTO PR_DashboardSummaryByOwnerQ (OwnerUserNo,NumberOfProspects,LastStageNumber) VALUES (" & rsAllOwners("OwnerUserNo")
						...
						SQLSummary1 = "SELECT * FROM PR_DashboardSummaryByOwnerQ"
						Set rsSummary = cnnBuildSQL4.Execute(SQLSummary1) 
						...
						If NOT rsSummary.EOF Then
							...
							Do While Not rsSummary.EOF
								...
								SQLtmp = "SELECT COUNT(*) AS Expr1 FROM PR_DashboardDetailsQ WHERE OwnerUserno = " & rsSummary("OwnerUserNo") & " AND LastStageNumber  = " & rsSummary("LastStageNumber")
								'Response.Write(SQLtmp & "<BR>")
								Set rsTmp1 = cnnBuildSQL4.Execute(SQLtmp)	
								NOP = rsTmp1("Expr1")
								
								SQLtmp = "UPDATE PR_DashboardSummaryByOwnerQ SET  NumberOfProspects = " & NOP & " WHERE OwnerUserno = " & rsSummary("OwnerUserNo") & " AND LastStageNumber  = " & rsSummary("LastStageNumber")
								Set rsTmp1 = cnnBuildSQL4.Execute(SQLtmp)	
						...
						'************************************
						'Rebuild PR_DashboardSummaryByLSourceQ
						'************************************
						Set cnnSummary = Server.CreateObject("ADODB.Connection")
						cnnSummary.CursorLocation = adUseClient
						cnnSummary.open (Session("ClientCnnString"))
						...
						objRecordSet.CursorLocation = 3
						objConnection.Open (Session("ClientCnnString"))
						objSQL = "SELECT * FROM sysobjects Where Name= 'PR_DashboardSummaryByLSourceQ' AND xType= 'U'"
						objRecordSet.Open objSQL,objConnection
						
						If objRecordset.RecordCount = 0  Then
							'Not there, we must create it
							SQLSummary = "CREATE TABLE PR_DashboardSummaryByLSourceQ "
							SQLSummary = SQLSummary & "	( "
							SQLSummary = SQLSummary & "     [InternalRecordIdentifier] [int] IDENTITY(1,1) NOT NULL, "
							SQLSummary = SQLSummary & "     [RecordCreationDateTime] [datetime] NULL, "
							SQLSummary = SQLSummary & "     [LeadSourceNumber] [int] NULL, "
						...
						Else
							'It is there, so just empty it
							SQLSummary = "DELETE FROM PR_DashboardSummaryByLSourceQ"
							Set rsSummary = cnnSummary.Execute(SQLSummary)
						End If
						Set objRecordSet = Nothing
						objConnection.Close
						...
						rsAllLSource.CursorLocation = 3 
						rsAllLSource.CursorType = 3
						SQLAllLSource = "SELECT DISTINCT LeadSourceNumber,* FROM PR_DashboardDetailsQ"
						Set rsAllLSource = cnnBuildSQL3.Execute(SQLAllLSource) 
						...
						IF Not rsAllLSource.EOF Then		
							Do While Not rsAllLSource.EOF
									...
									SQLSummary1 = "INSERT INTO PR_DashboardSummaryByLSourceQ (LeadSourceNumber,NumberOfProspects,LastStageNumber) VALUES (" & rsAllLSource("LeadSourceNumber")
						...
						SQLSummary1 = "SELECT * FROM PR_DashboardSummaryByLSourceQ"
						Set rsSummary = cnnBuildSQL4.Execute(SQLSummary1) 
						...
						If NOT rsSummary.EOF Then
							...
							Do While Not rsSummary.EOF
								...
								SQLtmp = "SELECT COUNT(*) AS Expr1 FROM PR_DashboardDetailsQ WHERE LeadSourceNumber = " & rsSummary("LeadSourceNumber") & " AND LastStageNumber  = " & rsSummary("LastStageNumber")
								Set rsTmp1 = cnnSummary.Execute(SQLtmp)	
								NOP = rsTmp1("Expr1")
								
								SQLtmp = "UPDATE PR_DashboardSummaryByLSourceQ SET  NumberOfProspects = " & NOP & " WHERE LeadSourceNumber = " & rsSummary("LeadSourceNumber") & " AND LastStageNumber  = " & rsSummary("LastStageNumber")
								Set rsTmp1 = cnnSummary.Execute(SQLtmp)	
