...
<SCRIPT LANGUAGE="JavaScript">
    function validatewaitForPartsform()
    {
        if (document.frmwaitForParts.selAssetID.value != "") {
      	  if (document.frmwaitForParts.txtAssetTagNumber.value != "") {
        ...
        if (document.frmwaitForParts.selAssetID.value != "") {
      	  if (document.frmwaitForParts.txtAssetLocation.value == "") {
...
Wait For Parts Ticket # <%=SelectedMemoNumber%></h1>
 

<div class="container-fluid fieldservice-container">
	<div class="row">
 
		<form action="waitForParts_submit.asp" method="POST" ENCTYPE="multipart/form-data" name="frmwaitForParts" id="frmwaitForParts" onsubmit="return validatewaitForPartsform();"> 
		...
		<input type='hidden' id='txtTicketNumber' name='txtTicketNumber' value='<%=SelectedMemoNumber%>'>		 
			...
			<div class="row row-rest">
				<h5 class="close-service-h4"><i class="fa fa-comments"></i> Enter the details regarding the part(s) you need in the box below. An email will automatically be sent to the service mangers with your parts request.</h5>
