...
<%
dummy = MUV_Write("selectedServiceTab","#awaitingdispatch")
...
<div class="table-responsive">
    <table  id="tableSuperSum1" class="food_planner table table-condensed sortable">
      <thead>
        <tr>
	...
	Else
		SQL = "SELECT * FROM FS_ServiceMemos WHERE CurrentStatus = 'OPEN' AND RecordSubType = 'OPEN' AND FilterChange <> 1"
		SQL = SQL & "ORDER BY submissionDateTime DESC"
	End If
	
	Set rs = cnn8.Execute(SQL)

	DynamicFormCounter = 100
	If not rs.EOF Then
		LineX=1
		Do While Not rs.EOF
			...
			If ShowThisRec = True AND rs.Fields("RecordSubType") <> "HOLD" AND (GetServiceTicketCurrentStageVar = "Received" OR GetServiceTicketCurrentStageVar = "Released" OR GetServiceTicketCurrentStageVar = "Declined") Then
